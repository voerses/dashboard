"""M6 — MultiInstrumentCache 3-tuple key + memory budget (T-D1, T-D1a, T-D1b).

All tests MUST FAIL today — v5.data.cache does not exist.
"""
...
class TestMultiInstrumentCacheThreeTupleKey:
    ...
    def test_same_stream_distinct_roles_distinct_caches(self):
        ...
        inst = _inst(); spec = BarSpec.from_minutes(60)
        for i in range(3):
            cache.on_bar(_mk_bar(i * spec.period_ns, 100.0 + i, spec, inst), role="signal")
        cache.on_bar(_mk_bar(99 * spec.period_ns, 999.0, spec, inst), role="entry")
        stream = DataStream(instrument=inst, data_kind=DataKind.BAR, bar_spec=spec)
    ...
    def test_role_driven_lookback_applied(self, role):
        ...
        for i in range(expected + 10):
            cache.on_bar(_mk_bar(i * spec.period_ns, 100.0, spec, inst), role=role)
        stream = DataStream(instrument=inst, data_kind=DataKind.BAR, bar_spec=spec)
        assert len(cache.arrays(stream, role=role).close) == expected


class TestMultiInstrumentCacheMemoryBudget:
        ...
        for i in range(236):
            ...
            for spec in specs:
                for role in ("signal", "entry", "exit"):
                    cache.on_bar(_mk_bar(0, 100.0, spec, inst), role=role)
    ...
    def test_projection_matches_m4_arithmetic(self):
        """Cross-check: M6's MultiInstrumentCache.compute_projected_memory_mb()
        ...
        cache.on_bar(_mk_bar(0, 100.0, spec_1m, iid), role="signal")
        cache.on_bar(_mk_bar(0, 100.0, spec_1h, iid), role="entry")
        expected_bytes = (
    ...
    def test_compute_projected_memory_mb_returns_float(self):
        from v5.data.cache import MultiInstrumentCache
        mb = MultiInstrumentCache().compute_projected_memory_mb()
    ...
    def test_project_from_subscriptions_eager(self):
        ...
        from v5.data.streams import (
            DataKind, DataStream, InstrumentId, Subscription, Venue,
        )
        subs: list = []
        for i in range(50):
            inst = InstrumentId(symbol=f"TOK{i:03d}USDT", venue=Venue.BINANCE, asset_class="perp")
            stream = DataStream(instrument=inst, data_kind=DataKind.BAR,
                                bar_spec=BarSpec.from_minutes(60))
            subs.append(Subscription(stream=stream, handler=lambda b: None, role="signal"))
        projected = MultiInstrumentCache().project_from_subscriptions(subs)
    ...
    def test_rejects_duplicate_ts_event(self):
        ...
        cache = MultiInstrumentCache()
        inst = _inst(); spec = BarSpec.from_minutes(60)
        cache.on_bar(_mk_bar(10 * spec.period_ns, 100.0, spec, inst), role="signal")
        with pytest.raises(ValueError):
            cache.on_bar(_mk_bar(10 * spec.period_ns, 101.0, spec, inst), role="signal")
