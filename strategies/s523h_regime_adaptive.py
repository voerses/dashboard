...
#       --strategy s523h_regime_adaptive --months 51 --capital 50000 \
#       --market perp --conviction-mode ranked \
#       --max-portfolio-positions 30 --skip-wf \
#       --end-date 2026-04-05
#
S523c_regime — Regime-Adaptive Composite Positioning
====================================================

Based on s523c_growth with ONE addition: BTC 200-day SMA regime detection.
When BTC is BELOW its 200-day SMA (bear regime), the composite sign is
FLIPPED so the strategy trend-follows with the crowd instead of fading it.

Research: post-hoc regime sweep on 51-month engine trades showed this
single change eliminates both losing years (2022: -22%→+24%, 2023: -32%→+21%)
while keeping 2024 strong (+61%→+101%) at the cost of reducing 2025 from
+747% to +280%. Multi-timeframe monthly candle context was tested and found
NOT to improve on the simple SMA-200 binary.
...
THRESHOLD = 0.5               # composite z-score threshold for entry (lower than s520)
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    # ---- BTC 200-day SMA regime detection ----
    # When BTC is below its 200-day SMA → bear regime → FLIP composite sign.
    # In bear markets, "crowded positioning" means continuation (not reversal),
    # so we trend-follow WITH the crowd instead of fading it.
    # In bull markets, the contrarian stance is correct (crowded → reversal).
    # The SMA is computed from the SAME 1h close data the engine already loads.
    BTC_SMA_DAYS = 200
    BTC_SMA_BARS = BTC_SMA_DAYS * 24  # 200 days in hourly bars
    try:
        ...
        btc_close_full = _get_composite_aligned._btc_close_cache
        # Align BTC close to the strategy's 1h index
        btc_aligned = btc_close_full.reindex(ctx.idx_1h, method="ffill")
        btc_sma_200d = btc_aligned.rolling(BTC_SMA_BARS, min_periods=BTC_SMA_BARS // 2).mean()
        bear_regime = (btc_aligned < btc_sma_200d).values
        # Flip composite in bear regime
        regime_flip = np.where(bear_regime, -1.0, 1.0)
        adjusted_composite = adjusted_composite * regime_flip
    except Exception:
        pass  # If BTC data unavailable, fall through without flip (safe fallback)
