"""Strategy S277: ML Alt-Liquid Concentrated (Long, Perp, 3x)
=============================================================
KEY INSIGHT: ETH/SOL/DOGE/XRP have massive ML edge (+1.6% to +3.1%
at p>=0.55), far exceeding costs (~0.3-0.5%). BTC edge (+0.41%)
is too thin after costs — removed from whitelist.
FIX from s276: Added 10 ATR catastrophic stop (activating after
18h) to prevent liquidations that destroyed -$63K.
"""
import numpy as np
import pandas as pd
import joblib
...
# No BTC — too thin edge after costs at p>=0.55
_WHITELIST = {'ETH', 'SOL', 'DOGE', 'XRP'}
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    if token and token not in _WHITELIST:
        return StrategyResult(
            ...
            max_trade_pct=0.35, exchange='binance', name='s277_ml_alt_liquid')
    ...
    return StrategyResult(
        ...
        market_type=MarketType.PERP, leverage=3.0,
        stop_mult=10.0,       # Catastrophic stop only (10 ATR ~30-40%)
        trail_mult=999.0,
        target_mult=999,
        no_stop_bars=18,      # Only active in last 6h of hold
        ...
        exchange='binance', name='s277_ml_alt_liquid')
