"""Strategy S281: ML Alt-Liquid Trend-Filtered (Long, Perp, 3x)
=============================================================
Same as s277 but also filters DOWNTREND regime entries.
Hypothesis: Long-only ML signals during downtrends produce the
biggest losing streaks. Filtering them should cut DD while
sacrificing some profitable trades.
Also uses higher threshold for volatile tokens (DOGE/XRP p>=0.58).
"""
import numpy as np
import pandas as pd
import joblib
...
_HIGH_VOL = {'DOGE', 'XRP'}  # Require higher confidence
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    if token and token not in _WHITELIST:
        return StrategyResult(
            ...
            max_trade_pct=0.35, exchange='binance', name='s281_ml_trend_filtered')
    ...
    # Token-specific threshold: higher for volatile tokens
    threshold = 0.58 if token in _HIGH_VOL else 0.55

    # REGIME FILTER: No entries during CRISIS or DOWNTREND
    ...
    return StrategyResult(
        ...
        exchange='binance', name='s281_ml_trend_filtered')
