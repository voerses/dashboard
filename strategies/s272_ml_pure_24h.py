"""Strategy S272: ML Direction Pure 24h Hold (Long-Only, Perp, 1x)
================================================================
CRITICAL FINDING: Stop exits destroyed -$110K while max_hold exits
made +$36K. Solution: NO stops at all, pure 24h hold-and-exit.
The ML model predicts close[t+24]/close[t], so hold exactly 24h.
"""
import numpy as np
import pandas as pd
import joblib
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    return StrategyResult(
        ...
        market_type=MarketType.PERP, leverage=1.0,
        # NO STOPS — pure 24h hold-and-exit
        stop_mult=999.0,      # Effectively disabled
        trail_mult=999.0,     # Disabled
        target_mult=999,      # Disabled
        no_stop_bars=999,     # Stops never activate
        min_hold=24, max_hold=24,  # Exact 24h hold
        edge=0.80,
        exit_regimes=set(),   # NO regime exits either
        breakeven_atr=999.0,
        max_trade_pct=0.20,
        conviction_score=conviction,
        exchange='binance', name='s272_ml_pure_24h')
