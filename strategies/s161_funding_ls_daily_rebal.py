"""
s161 Volatile Funding L/S — Daily Rolling Rebalance — V4 Portfolio Strategy (Class B)

Recycle of s151: replace fixed 7-day rebalance with daily (24h).
s151's REBALANCE_BARS was critically fragile: both 5-day and 10-day lost money.
Daily rebalance should smooth out the timing dependency and make the strategy
less sensitive to the exact rebalance cadence.

Tradeoff: more turnover (higher fees) but more responsive signal.
...
FUNDING_WINDOW = 24
REBALANCE_BARS = 24                  # Changed from 168 (7d) to 24 (1d)
N_LONG = 5
N_SHORT = 5
...
def strategy(contexts: dict) -> dict:
    """Cross-sectional funding L/S on volatile tokens — daily rebalance."""
    token_data = {}
    ...
    for j, token in enumerate(eligible_tokens):
        ...
        results[token] = StrategyResult(
            ...
            exit_regimes=set(),
            exchange='binance',
            name='s161_funding_ls_daily_rebal',
            breakeven_atr=0.0,
        )
