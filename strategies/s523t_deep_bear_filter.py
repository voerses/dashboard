...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    # Deep bear long suppression: no longs when BTC monthly return < -5%
    # In confirmed down months, contrarian 'buy the dip' longs are losers.
    # Blocking them saves capital AND frees slots for profitable shorts.
    # Research: +1,295pp across all years (2022-2026).
    try:
        _deep_bear = np.nan_to_num(_m_ret_1mo_h, nan=0) < -0.05
