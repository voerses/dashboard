"""Strategy S294: ML Short Aggressive (Short, Perp, 5x, cap_mult=1.5)
====================================================================
More aggressive short hedge: 5x leverage, cap_mult=1.5.
Shorts during DOWNTREND + CRISIS regimes.
The short strategy's DD occurs during long's gain periods and vice versa.
Bigger shorts = bigger hedge = more DD reduction on the portfolio level.
"""
import numpy as np
import pandas as pd
import joblib
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    if token and token not in _WHITELIST:
        return StrategyResult(
            ...
            exchange='binance', name='s294_ml_short_aggressive')
    ...
    return StrategyResult(
        ...
        market_type=MarketType.PERP, leverage=5.0,    # 5x leverage on shorts
        stop_mult=10.0,
        trail_mult=999.0,
        target_mult=999,
        no_stop_bars=18,
