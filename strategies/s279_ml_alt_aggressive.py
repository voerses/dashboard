"""Strategy S279: ML Alt-Liquid Aggressive Sizing (Long, Perp, 3x)
================================================================
Same signal as s277, but cap_multiplier=3.0 to overcome the
conservative ADV-based cap_pct (~10-12% of equity).
On ultra-liquid tokens, the default cap is extremely conservative.
Position sizes ~$55-70K margin vs $20-27K default.
"""
import numpy as np
import pandas as pd
import joblib
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    if token and token not in _WHITELIST:
        return StrategyResult(
            ...
            max_trade_pct=0.50, cap_multiplier=3.0,
            exchange='binance', name='s279_ml_alt_aggressive')
    ...
    return StrategyResult(
        ...
        trail_mult=999.0,
        target_mult=999,
        no_stop_bars=18,      # Active in last 6h only
        min_hold=24, max_hold=24,
        edge=2.00,
        exit_regimes=set(),
        breakeven_atr=999.0,
        max_trade_pct=0.50,   # Allow up to 50% of equity
        cap_multiplier=3.0,   # 3x the ADV-based cap → $55-70K positions
