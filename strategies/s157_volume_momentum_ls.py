"""
s157 Volume Momentum Ranking L/S — V4 Portfolio Strategy (Class B)

Alternative signal for volatile token cross-sectional L/S.
Instead of funding, rank by 7-day volume change (volume_now / volume_7d_ago).

Thesis: Tokens with massive volume spikes are experiencing blow-off tops
(short them). Tokens with declining volume are coiling for a squeeze (long them).
...
def strategy(contexts: dict) -> dict:
    """Cross-sectional volume momentum L/S on volatile tokens."""
    token_data = {}
    for token, ctx_pair in contexts.items():
        ...
        # Volume momentum: rolling mean now / rolling mean 7d ago
    ...
    # Build volume momentum matrix: vol_now / vol_7d_ago (vectorized)
    vol_mom_matrix = np.full((max_bars, n_tokens), np.nan, dtype=np.float64)
    for j, token in enumerate(eligible_tokens):
        d = token_data[token]
        n = d['n']
        offset = max_bars - n
        vol_ma = d['vol_ma']
        # Vectorized: ratio of current vol_ma to vol_ma shifted by VOL_LOOKBACK
        cur = vol_ma[VOL_LOOKBACK:]
        prev = vol_ma[:n - VOL_LOOKBACK]
        vol_mom = np.full(n, np.nan, dtype=np.float64)
        mask = (prev > 0) & ~np.isnan(prev) & ~np.isnan(cur)
        vol_mom[VOL_LOOKBACK:] = np.where(mask, cur / np.maximum(prev, 1e-20), np.nan)
        vol_mom_matrix[offset:offset + n, j] = vol_mom
    ...
    for rb in rebalance_bars:
        ...
        # Sort ascending by volume momentum
        # Low vol_mom = declining volume = long (coiling for squeeze)
        # High vol_mom = volume spike = short (blow-off top)
