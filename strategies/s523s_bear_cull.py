...
#       --strategy s523j_v2_proven --months 12 --capital 100000 \
...
# Mission P breadth cull: active in bear years only (2025, 2026, 2029, 2030...)
# Applied as overlay via tools/s523c_breadth_cull_monitor.py in live,
# or tools/s523d_run_with_cull.py in backtest.
# When 80%+ of a direction's post-grace positions are underwater, close that basket.
# Params: grace_days=3, breadth_threshold=0.80, throttle_days=14, cost_bps=50
MISSION_P_CULL = True  # enables breadth cull overlay
MISSION_P_BEAR_YEARS_ONLY = True  # only active in bear years (yr1+yr2 after halving)
STOP_MULT = 5.0
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    # Simple year-based gate: bear years = shorts ungated, bull years = 1mo_red
    # Bull years: halving year + year before (2023, 2024)
    # Bear years: 2 years after halving year (2025, 2026)
    # This follows the ~4yr cycle: -1yr/halving_yr = bull, +1yr/+2yr = bear
    _bar_years = np.array([t.year for t in ctx.idx_1h])
    # 2020 cycle: bull=2019,2020 bear=2021,2022
    # 2024 cycle: bull=2023,2024 bear=2025,2026
    _bear_years = np.isin(_bar_years, [2021, 2022, 2025, 2026, 2029, 2030])
