"""Strategy S309: ML Adaptive (Long+Short, Perp, regime-aware)
====================================================================
Single strategy that adapts to market regime:
  UPTREND/QUIET:  Go long,  5x leverage, size_mult=2.1
  DOWNTREND:      Go short, 4x leverage, size_mult=1.3
  RANGE:          Go short, 3x leverage, size_mult=0.7 (reduced)
  CRISIS:         Sit out (no entries)

Uses the same ML model for both long and short probability.
Per-bar arrays for direction, leverage, and size_multiplier.
"""
import numpy as np
import pandas as pd
import joblib
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    if token and token not in _WHITELIST:
        return StrategyResult(
            ...
            cap_multiplier=1.0, size_multiplier=np.ones(n),
            max_trade_pct=0.50,
            exchange='binance', name='s309_ml_adaptive')
    ...
    short_threshold = 0.60 if token in _HIGH_VOL else 0.55

    # --- Per-bar regime-adaptive signals ---
    # Now uses per-bar cap_multiplier: each regime gets its own sizing cap.
    entry_mask = np.zeros(n, dtype=bool)
    direction = np.zeros(n, dtype=np.int8)
    leverage = np.ones(n, dtype=np.float64) * 5.0
    cap_mult = np.ones(n, dtype=np.float64)

    # UPTREND or QUIET: go long, 5x leverage, cap_mult=1.05
    # (2-strat had weight=0.5 * cap_mult=2.1 = effective 1.05)
    long_regime = (regime == UPTREND) | (regime == QUIET)
    long_signal = long_regime & (long_prob >= threshold)
    entry_mask[long_signal] = True
    direction[long_signal] = 1
    leverage[long_signal] = 5.0
    cap_mult[long_signal] = 1.05

    # DOWNTREND: go short, 4x leverage, cap_mult=0.65
    # (2-strat had weight=0.5 * cap_mult=1.3 = effective 0.65)
    short_down = (regime == DOWNTREND) & (short_prob >= short_threshold)
    entry_mask[short_down] = True
    direction[short_down] = -1
    leverage[short_down] = 4.0
    cap_mult[short_down] = 0.65

    # RANGE: no shorts — chop kills edge (Dec 2025: -$121k)
    # Sit out and preserve capital

    # CRISIS: no entries (sit out)
    ...
    # Conviction: use max of relevant probability
    ...
    return StrategyResult(
        ...
        exit_regimes=set(),
        breakeven_atr=999.0,
        cap_multiplier=cap_mult,       # per-bar: 2.1x long, 1.3x short, 0.7x range
        max_trade_pct=0.50,
