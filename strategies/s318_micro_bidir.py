...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    empty = StrategyResult(
        ...
        exchange='binance', name='s318_micro_bidir')
    ...
    # V3: Bidirectional, no stops, 1x leverage — pure model test
    short_entry = short_prob >= 0.65
    long_entry = long_prob >= 0.60

    direction = np.ones(n, dtype=np.int8)
    direction[short_entry] = -1
    entry_mask = short_entry | long_entry
    ...
    return StrategyResult(
        ...
        edge=1.50, exit_regimes=set(), breakeven_atr=999.0,
        cap_multiplier=1.0, max_trade_pct=0.15,
        conviction_score=np.maximum(short_prob, long_prob),
        exchange='binance', name='s318_micro_bidir')
