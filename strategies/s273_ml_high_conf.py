"""Strategy S273: ML Direction High-Confidence Only (Long, Perp, 2x)
=================================================================
Higher threshold p>=0.65 for better signal quality.
2x leverage with very high stop (12 ATR) for catastrophic protection only.
Focus on highest-ADV tokens to minimize slippage.
"""
import numpy as np
import pandas as pd
import joblib
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    # HIGH CONFIDENCE: p>=0.65 for better edge per trade
    entry_long = (long_prob >= 0.65) & (regime != CRISIS)

    # Strict ADV filter: >$50M only (top ~67 tokens)
    ...
    adv_filter = rolling_dv > 50_000_000
    ...
    return StrategyResult(
        ...
        trail_mult=999.0,     # No trailing
        target_mult=999,
        no_stop_bars=999,
        min_hold=24, max_hold=24,  # Exact 24h hold
        edge=0.60,            # Moderate sizing
        exit_regimes=set(),   # No regime exits
        breakeven_atr=999.0,
        max_trade_pct=0.15,
        conviction_score=conviction,
        exchange='binance', name='s273_ml_high_conf')
