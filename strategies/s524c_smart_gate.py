...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    try:
        ...
        # G45 short gate with SMART regime switching:
        # Normal: shorts when BTC 45d return < 0
        # BTC dominance (BTC up, TOTAL2 down): shorts when TOTAL2 45d return < 0
        _m_ret_45d_h = (btc_aligned / btc_aligned.shift(45 * 24) - 1).values
        _m_ret_45d_h = np.nan_to_num(_m_ret_45d_h, nan=0)
        _short_ok_btc = (_m_ret_45d_h < 0)

        # Load TOTAL2 for smart gate
        _t2_path = os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
            "data", "alternative", "total2_total3.parquet",
        )
        if not hasattr(_get_composite_aligned, '_total2_cache'):
            _get_composite_aligned._total2_cache = None
        if _get_composite_aligned._total2_cache is None and os.path.exists(_t2_path):
            _t2_df = pd.read_parquet(_t2_path)
            if _t2_df.index.tz is not None:
                _t2_df.index = _t2_df.index.tz_localize(None)
            _get_composite_aligned._total2_cache = _t2_df['total2_close']

        _t2_close = _get_composite_aligned._total2_cache
        if _t2_close is not None:
            _t2_aligned = _t2_close.reindex(ctx.idx_1h.normalize(), method='ffill')
            _t2_aligned.index = ctx.idx_1h
            _t2_45d = (_t2_aligned / _t2_aligned.shift(45 * 24) - 1).values
            _t2_45d = np.nan_to_num(_t2_45d, nan=0)
            _short_ok_t2 = (_t2_45d < 0)

            # BTC dominance: BTC 45d positive AND TOTAL2 45d negative
            _btc_dom = (_m_ret_45d_h > 0) & (_t2_45d < 0)

            # SMART gate: use TOTAL2 gate during BTC dominance in HALVING YEARS only
            # Halving years (2020, 2024, 2028): BTC ETF/institutional flows cause
            # structural BTC dominance where alts bleed despite BTC rallying.
            # In non-halving years, BTC dominance is just rotation (alts follow later).
            _halving_yrs = np.isin(np.array([t.year for t in ctx.idx_1h]), [2020, 2024, 2028, 2032])
            _smart_override = _btc_dom & _halving_yrs
            _short_ok = np.where(_smart_override, _short_ok_t2, _short_ok_btc)
        else:
            _short_ok = _short_ok_btc
