...
#       --strategy s523m_conviction_adaptive --months 12 --capital 100000 \
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    if ticker in TOKEN_BLACKLIST or ticker not in _token_configs:
        return StrategyResult(
            ...
            max_hold=720,
            edge=0.0,
            name='s523m_conviction_adaptive',
    ...
    # v2 proven + dilution filter + conviction weighting
    ...
    # s523m: DYNAMIC conviction weighting by regime
    # Longs: boost in trending-up (from v2)
    _lm = direction == 1
    _sm = direction == -1
    conviction[_lm] *= _long_conv[_lm]
    # Shorts: boost when BTC momentum is decelerating (30d ret < 90d ret)
    # This detects bear reversals without binary gating
    try:
        _r30 = (btc_aligned / btc_aligned.shift(30*24) - 1).reindex(ctx.idx_1h, method='ffill').values
        _r90 = (btc_aligned / btc_aligned.shift(90*24) - 1).reindex(ctx.idx_1h, method='ffill').values
        _momentum_decay = np.nan_to_num(_r30, nan=0) < np.nan_to_num(_r90, nan=0)
        # Also: check if BTC is below daily SMA200 (bear confirmed)
        _sma200d = btc_aligned.rolling(200*24, min_periods=100*24).mean()
        _bear_confirmed = (btc_aligned < _sma200d).values
        # Short conviction multiplier:
        # - Bear confirmed + momentum decaying: 1.5x (strong short signal)
        # - Momentum decaying only: 1.2x (early bear signal)
        # - Neither: 0.7x (bull → shorts deprioritized but not blocked)
        _short_conv = np.full(n, 0.7)
        _short_conv[_momentum_decay] = 1.2
        _short_conv[_momentum_decay & _bear_confirmed] = 1.5
