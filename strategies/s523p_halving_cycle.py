...
#       --strategy s523p_halving_cycle --months 12 --capital 100000 \
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    # s523p: 1mo_red gate + dilution + halving conviction
    ...
    # ==== s523p: HALVING CYCLE conviction overlay ====
    # BTC halvings: deterministic, every ~4 years
    # Cycle phases (months since halving):
    #   0-12:  BULL       → long conv 1.3, short conv 0.5
    #   12-18: TRANSITION → long conv 0.8, short conv 1.0
    #   18-30: BEAR       → long conv 0.5, short conv 1.3
    #   30-48: RECOVERY   → long conv 1.0, short conv 0.7
    _lm = direction == 1
    _sm = direction == -1
    try:
        ...
        # Find months since most recent halving for each bar
        _halving_months = np.zeros(n, dtype=np.float64)
        for _i, _ts in enumerate(ctx.idx_1h):
            _last_h = max(h for h in _halvings if h <= _ts) if any(h <= _ts for h in _halvings) else _halvings[0]
            _halving_months[_i] = (_ts - _last_h).days / 30.44
        # Map to conviction multipliers
        _h_long_conv = np.ones(n, dtype=np.float64)
        _h_short_conv = np.ones(n, dtype=np.float64)
        # Phase 1: Bull (0-12 months)
        _bull = _halving_months < 12
        _h_long_conv[_bull] = 1.3
        _h_short_conv[_bull] = 0.5
        # Phase 2: Transition (12-18)
        _trans = (_halving_months >= 12) & (_halving_months < 18)
        _h_long_conv[_trans] = 0.8
        _h_short_conv[_trans] = 1.0
        # Phase 3: Bear (18-30)
        _bear = (_halving_months >= 18) & (_halving_months < 30)
        _h_long_conv[_bear] = 0.5
        _h_short_conv[_bear] = 1.3
        # Phase 4: Recovery (30-48)
        _recov = _halving_months >= 30
        _h_long_conv[_recov] = 1.0
        _h_short_conv[_recov] = 0.7
        # Apply: multiply with existing v2 long_conv (trending-up boost)
        conviction[_lm] *= _long_conv[_lm] * _h_long_conv[_lm]
        conviction[_sm] *= _h_short_conv[_sm]
    except Exception:
