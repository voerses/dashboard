"""Strategy S287: GradientBoosting ML Long (Long, Perp, 3x)
=============================================================
Independent second ML model (GradientBoosting) for signal diversity.
Same token universe and regime filter as s282, but uses GB model
which has different feature importances (vol_50, funding_ma48, ret_168
vs RF's more diverse feature set). Adds uncorrelated trade timing.

GB Model Stats:
  p>=0.55: 61.1% precision (LONG), p>=0.60: 67.9%, p>=0.65: 74.6%
"""
import numpy as np
import pandas as pd
import joblib
...
_model = joblib.load(os.path.join(_MODEL_DIR, 'ml_dir_gb_model.joblib'))
_scaler = joblib.load(os.path.join(_MODEL_DIR, 'ml_dir_gb_scaler.joblib'))
_feat_names = joblib.load(os.path.join(_MODEL_DIR, 'ml_dir_gb_features.joblib'))
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    if token and token not in _WHITELIST:
        return StrategyResult(
            ...
            max_trade_pct=0.35, exchange='binance', name='s287_ml_gb_long')
    ...
    threshold = 0.60 if token in _HIGH_VOL else 0.58
    ...
    return StrategyResult(
        ...
        exchange='binance', name='s287_ml_gb_long')
