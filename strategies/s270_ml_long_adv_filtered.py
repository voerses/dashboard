"""Strategy S270: ML Direction Long-Only, ADV-Filtered (Perp, 3x)
================================================================
Key research findings:
- ML LONG p>=0.60 has +1.58% raw edge on high-ADV tokens (58.9% win rate)
- SHORT signals degraded in 2025-2026 (18% precision) — excluded
- Slippage kills edge on low-ADV tokens: only trade >$30M ADV
- 3x leverage with fixed 24h hold matches prediction horizon
"""
import numpy as np
import pandas as pd
import joblib
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    # LONG-ONLY with ADV filter
    entry_long = (long_prob >= 0.60) & (regime != CRISIS)

    # ADV filter: only trade tokens where the volume supports our position
    # Use rolling dollar volume as proxy (close * volume * 24 = daily volume)
    rolling_dv = np.zeros(n)
    dv_hourly = close * volume
    # 20-day rolling average daily volume
    for i in range(20*24, n):
        rolling_dv[i] = np.mean(dv_hourly[i-20*24:i]) * 24
    # For early bars, use available data
    for i in range(24, 20*24):
        rolling_dv[i] = np.mean(dv_hourly[:i]) * 24

    # Only trade when ADV > $30M (where costs are manageable)
    adv_filter = rolling_dv > 30_000_000
    entry_long = entry_long & adv_filter

    entry_mask = entry_long
    direction = np.ones(n, dtype=np.int8)  # Always LONG
    ...
    return StrategyResult(
        ...
        stop_mult=8.0,        # Wide emergency stop only
        trail_mult=999.0,     # No trailing stop — hold to horizon
        target_mult=999,      # No target — exit at max_hold
        no_stop_bars=24,      # No stop during prediction horizon
        min_hold=20, max_hold=28,  # Fixed ~24h hold
        edge=0.80,            # Aggressive sizing for high-ADV tokens
        exit_regimes={CRISIS},
        breakeven_atr=999.0,  # No breakeven
        max_trade_pct=0.20,   # Allow larger positions
        conviction_score=conviction,
        exchange='binance', name='s270_ml_long_adv_filtered')
