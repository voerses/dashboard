...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    # ---- Per-bar adaptive stop: wider during BTC dominance ----
    # BTC dominance = longs are RIGHT but get stopped out prematurely
    # Wider stops (7 ATR) give longs room to survive the initial drawdown
    _stop_arr = np.full(n, STOP_MULT, dtype=np.float64)
    try:
        _t2_sigs_stop = _load_total2_signals(ctx.idx_1h, btc_aligned)
        _btc_dom_stop = _t2_sigs_stop["btc_dom_30d"]
        # BTC dominance: widen stops for longs (7 ATR), keep normal for shorts
        _dom_active = _btc_dom_stop > 0.05  # BTC outperforming TOTAL2 by >5%
        _long_mask = direction == 1
        _stop_arr[_dom_active & _long_mask] = 7.0
    except:
        pass
