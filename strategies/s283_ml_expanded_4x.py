"""Strategy S283: ML Expanded Trend-Filtered 4x Leverage (Long, Perp)
====================================================================
Same signal as s282 but 4x leverage instead of 3x.
With s282 DD at -17.9%, we have headroom to 20%. 4x leverage
should scale returns by ~33% while DD increases proportionally.
Expected: ~125% return with ~24% DD... or maybe less if non-linear.
"""
import numpy as np
import pandas as pd
import joblib
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    if token and token not in _WHITELIST:
        return StrategyResult(
            ...
            max_trade_pct=0.35, exchange='binance', name='s283_ml_expanded_4x')
    ...
    return StrategyResult(
        ...
        market_type=MarketType.PERP, leverage=4.0,   # 4x leverage (up from 3x)
        stop_mult=10.0,
        trail_mult=999.0,
        target_mult=999,
        no_stop_bars=18,
        ...
        exchange='binance', name='s283_ml_expanded_4x')
