"""Strategy S278: ML Alt-Liquid 12h Hold (Long, Perp, 3x)
========================================================
COMPANION to s277 — same tokens, higher confidence threshold,
12h hold for faster turnover. Different strategy_id allows
stacking positions on same token (engine limits per token+strategy).
"""
import numpy as np
import pandas as pd
import joblib
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    if token and token not in _WHITELIST:
        return StrategyResult(
            ...
            no_stop_bars=8, min_hold=12, max_hold=12,
            edge=1.50, exit_regimes=set(), breakeven_atr=999.0,
            max_trade_pct=0.25, exchange='binance', name='s278_ml_alt_12h')
    ...
    # Higher threshold for 12h hold — need stronger signal for shorter horizon
    ...
    return StrategyResult(
        ...
        market_type=MarketType.PERP, leverage=3.0,
        stop_mult=10.0,       # Catastrophic stop
        trail_mult=999.0,
        target_mult=999,
        no_stop_bars=8,       # Active in last 4h
        min_hold=12, max_hold=12,  # 12h hold — 2x turnover
        edge=1.50,
        exit_regimes=set(),
        breakeven_atr=999.0,
        max_trade_pct=0.25,
        conviction_score=conviction,
        exchange='binance', name='s278_ml_alt_12h')
