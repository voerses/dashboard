"""Strategy S289: RF+GB Ensemble 7x (Long, Perp)
====================================================================
BOTH RandomForest AND GradientBoosting must agree on long signal.
When two independent ML models agree, confidence is much higher.
RF p>=0.55 AND GB p>=0.55 → ensemble signal at 7x leverage.

Expected: ~100-120 trades (intersection of RF and GB), but each with
extremely high conviction → justifies 7x leverage + cap_mult=2.5.
Combined with s285 short hedge, DD should be manageable.
"""
import numpy as np
import pandas as pd
import joblib
...
# Load BOTH models
_rf_model = joblib.load(os.path.join(_MODEL_DIR, 'ml_dir_model.joblib'))
_rf_scaler = joblib.load(os.path.join(_MODEL_DIR, 'ml_dir_scaler.joblib'))
_rf_feat_names = joblib.load(os.path.join(_MODEL_DIR, 'ml_dir_features.joblib'))
_rf_classes = list(_rf_model.classes_)

_gb_model = joblib.load(os.path.join(_MODEL_DIR, 'ml_dir_gb_model.joblib'))
_gb_scaler = joblib.load(os.path.join(_MODEL_DIR, 'ml_dir_gb_scaler.joblib'))
_gb_feat_names = joblib.load(os.path.join(_MODEL_DIR, 'ml_dir_gb_features.joblib'))
_gb_classes = list(_gb_model.classes_)
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    if token and token not in _WHITELIST:
        return StrategyResult(
            ...
            market_type=MarketType.PERP, leverage=7.0,
            stop_mult=10.0, trail_mult=999.0, target_mult=999,
            no_stop_bars=18, min_hold=24, max_hold=24,
            edge=3.00, exit_regimes=set(), breakeven_atr=999.0,
            cap_multiplier=2.5, max_trade_pct=0.50,
            exchange='binance', name='s289_ml_ensemble_7x')
    ...
    # RF model prediction
    X_rf = np.column_stack([features.get(k, np.zeros(n)) for k in _rf_feat_names])
    X_rf = np.nan_to_num(X_rf, nan=0, posinf=0, neginf=0)
    X_rf_s = _rf_scaler.transform(X_rf)
    proba_rf = _rf_model.predict_proba(X_rf_s)
    rf_long = proba_rf[:, _rf_classes.index(1)] if 1 in _rf_classes else np.zeros(n)

    # GB model prediction
    X_gb = np.column_stack([features.get(k, np.zeros(n)) for k in _gb_feat_names])
    X_gb = np.nan_to_num(X_gb, nan=0, posinf=0, neginf=0)
    X_gb_s = _gb_scaler.transform(X_gb)
    proba_gb = _gb_model.predict_proba(X_gb_s)
    gb_long = proba_gb[:, _gb_classes.index(1)] if 1 in _gb_classes else np.zeros(n)

    # ENSEMBLE: both models must agree
    rf_thresh = 0.58 if token in _HIGH_VOL else 0.55
    gb_thresh = 0.58 if token in _HIGH_VOL else 0.55
    ...
    # Ensemble conviction: average of both models
    conviction = (rf_long + gb_long) / 2.0

    return StrategyResult(
        entry_mask=entry_mask, direction=direction,
        market_type=MarketType.PERP, leverage=7.0,   # 7x on ensemble signals
        stop_mult=10.0,
        trail_mult=999.0,
        target_mult=999,
        no_stop_bars=18,
        ...
        exit_regimes=set(),
        breakeven_atr=999.0,
        cap_multiplier=2.5,       # 2.5x position cap for high conviction
        max_trade_pct=0.50,
