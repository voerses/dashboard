"""Strategy S293: ML Conviction-Weighted Leverage (Long, Perp)
====================================================================
Dynamic per-bar leverage based on ML confidence:
  p >= 0.65: 8x leverage (highest conviction)
  p >= 0.60: 6x leverage
  p >= 0.55: 4x leverage

cap_multiplier=1.5 to avoid over-sizing.
Higher leverage on the best signals increases return without
proportionally increasing DD since high-conviction signals have
better win rates and larger forward returns.

Expected: fewer but larger winning trades dominate the return,
while lower-conviction trades are smaller and contribute less DD.
"""
import numpy as np
import pandas as pd
import joblib
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    if token and token not in _WHITELIST:
        return StrategyResult(
            ...
            cap_multiplier=1.5, max_trade_pct=0.50,
            exchange='binance', name='s293_ml_conviction_lever')
    ...
    # Per-bar conviction-weighted leverage
    leverage = np.where(long_prob >= 0.65, 8.0,
               np.where(long_prob >= 0.60, 6.0, 4.0)).astype(np.float64)

    # Size multiplier: scale position size with conviction
    size_mult = np.where(long_prob >= 0.65, 2.0,
                np.where(long_prob >= 0.60, 1.5, 1.0)).astype(np.float64)
