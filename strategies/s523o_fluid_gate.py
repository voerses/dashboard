...
#       --strategy s523o_fluid_gate --months 12 --capital 100000 \
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    # s523o: NO binary short gate — conviction handles it
    ...
    # ==== s523o: FLUID short/long conviction from BTC monthly return ====
    # Map BTC monthly return to continuous conviction multiplier
    # Strong green (+15%+) → short_conv=0.0 (blocked), long_conv=1.3
    # Mild green (+5%)    → short_conv=0.2 (rarely wins slot)
    # Flat (0%)           → short_conv=0.5
    # Mild red (-5%)      → short_conv=0.8
    # Strong red (-15%+)  → short_conv=1.3 (boosted)
    _lm = direction == 1
    _sm = direction == -1
    # BTC monthly return is in _m_ret_1mo_h (already hourly from v2 block)
    _mret = np.nan_to_num(_m_ret_1mo_h, nan=0)
    # Short conviction: linear map from +15% → 0.0, -15% → 1.3
    _short_conv = np.clip(0.65 - _mret * (0.65 / 0.15), 0.0, 1.3)
    # Long conviction: v2 trending-up boost + slight reduction in strong bear
    _long_conv_fluid = _long_conv.copy()  # starts from v2's trending-up boost
    _long_conv_fluid[_mret < -0.10] *= 0.7  # reduce longs in strong red months
