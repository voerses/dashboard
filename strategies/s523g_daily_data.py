...
#       --strategy s523c_growth --months 12 --capital 100000 \
...
_DAILY_PARQUET = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
    "data", "alternative", "binance_metrics", "all_symbols_daily_ls.parquet",
)

# Fallback: 5-min directory (used if daily parquet missing for a symbol)
_FIVEMIN_DIR = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
...
_daily_df: pd.DataFrame | None = None  # loaded once
...
def _ensure_daily_loaded():
    """Load the daily parquet once."""
    global _daily_df
    if _daily_df is not None:
        return
    if os.path.exists(_DAILY_PARQUET):
        _daily_df = pd.read_parquet(_DAILY_PARQUET)
        _daily_df["date"] = pd.to_datetime(_daily_df["date"])
        print(f"  [s523g] Loaded daily parquet: {len(_daily_df)} rows, "
              f"{_daily_df['symbol'].nunique()} symbols, "
              f"through {_daily_df['date'].max().strftime('%Y-%m-%d')}")
    else:
        _daily_df = pd.DataFrame()
        print(f"  [s523g] WARNING: Daily parquet not found at {_DAILY_PARQUET}")
...
def _load_daily_signals(symbol: str, ticker: str):
    """Load daily metrics from the combined parquet, compute per-token composite.
    Falls back to 5-min parquet if symbol missing from daily file.
    ...
    # Try daily parquet first
    sym_data = _daily_df[_daily_df["symbol"] == symbol] if _daily_df is not None and len(_daily_df) > 0 else pd.DataFrame()

    if len(sym_data) > 0:
        daily = sym_data.set_index("date").sort_index()
        # Map column names: daily parquet uses shortened names
        daily = daily.rename(columns={
            "sum_toptrader_ls_ratio": "sum_toptrader_long_short_ratio",
            "taker_buy_sell_ratio": "sum_taker_long_short_vol_ratio",
        })
    else:
        # Fallback: load from 5-min parquet
        parquet_path = os.path.join(_FIVEMIN_DIR, f"{symbol}_5min.parquet")
        ...
        try:
            ...
            print(f"  [s523g] WARNING: Failed to load {parquet_path}: {exc}")
            return
