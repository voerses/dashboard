"""
s41 Skew Trail Progression — s21 + O5 Progressive Trailing Stop

Wraps s21_skew_momentum with progressive trailing stop tightening.

Does NOT modify s21. Calls s21.strategy(), then adds trail_schedule.
...
from strategies.s21_skew_momentum import strategy as s21_strategy

TRAIL_SCHEDULE = np.array([
    [0.0, 3.0],   # Entry: match s21 base trail_mult
    [1.0, 2.5],
    [2.0, 2.0],
    [3.0, 1.5],
], dtype=np.float64)
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    """s21 skew momentum with progressive trailing stop."""
    result = s21_strategy(ctx)
