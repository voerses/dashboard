"""Strategy S310: ML Short DOWNTREND-only (Short, Perp, 4x, cap_mult=1.3)
====================================================================
DOWNTREND-only short (no RANGE, no CRISIS). Avoids chop losses (Dec 2025).
Higher threshold (0.60) for cleaner signals.
"""
import numpy as np
import pandas as pd
import joblib
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    if token and token not in _WHITELIST:
        return StrategyResult(
            ...
            exchange='binance', name='s310_ml_short_down_only')
    ...
    threshold = 0.60 if token in _HIGH_VOL else 0.60

    # Short during DOWNTREND ONLY — no RANGE (chop kills edge), no CRISIS (too volatile)
    ...
    return StrategyResult(
        ...
        exit_regimes=set(),
        breakeven_atr=999.0,
        cap_multiplier=1.3,       # 1.3x position cap
        max_trade_pct=0.35,
