"""
s103 MACD Squeeze Tight — Further DD reduction from s102

s102 achieved: +400.0% return, Sharpe 1.62, MaxDD -22.6%, Calmar 5.45
Still 2.6pp above DD target of <20%.

Changes from s102:
  1. TIGHTER TRAIL: 2.3 ATR (from 2.5) — modest tightening cuts tail losses
  2. MORE AGGRESSIVE VOL SCALING: 1.1x threshold (from 1.2x), 0.4 floor (from 0.5)
     Starts scaling earlier, goes lower during high-vol periods
  3. BREAKEVEN RATCHET: Kept at 0.5 ATR (same as s102)

All entry signals IDENTICAL to s98/s102.
...
# Trade management
STOP_MULT = 99.0
TRAIL_MULT = 2.3                # Tighter: 2.3 ATR (s98=2.5, s102=2.5)
TARGET_MULT = 999.0
NO_STOP_BARS = 72
MIN_HOLD = 24
MAX_HOLD = 720
...
# Volatility-scaled sizing (more aggressive than s102)
VOL_LOOKBACK = 168
VOL_SPIKE_THRESH = 1.1          # Start scaling at 1.1x (s102=1.2x)
VOL_MIN_SCALE = 0.4             # Floor at 40% (s102=50%)


def strategy(ctx: StrategyContext) -> StrategyResult:
    """MACD squeeze with tighter trail + aggressive vol scaling."""
