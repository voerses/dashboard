...
#       --strategy s523q_fixed --months 12 --capital 100000 \
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    # ==== s523q FIXED: halving cycle short gate ====
    # -1yr to +1yr around halving = BULL → use 1mo_red
    # Outside that window = BEAR → shorts ungated
    _halving_dates = [pd.Timestamp('2020-05-11'), pd.Timestamp('2024-04-20')]
    _in_bear_phase = np.ones(n, dtype=bool)  # default: bear (shorts ungated)
    for _hd in _halving_dates:
        _bull_start = _hd - pd.Timedelta(days=365)
        _bull_end = _hd + pd.Timedelta(days=365)
        # Use simple timestamp comparison (avoid DateOffset issues)
        for _bi in range(n):
            _bar_ts = ctx.idx_1h[_bi]
            if _bull_start <= _bar_ts <= _bull_end:
                _in_bear_phase[_bi] = False
    # In bull phase: 1mo_red gate. In bear phase: all shorts allowed.
    _short_ok_final = np.where(_in_bear_phase, True, _short_ok)
    # ==== end halving gate ====
    long_signal = long_signal & day_change & rsi_long_window & _dilution_ok_long & _dilution_ok_any
    short_signal = short_signal & day_change & rsi_short_window & _short_ok_final & _dilution_ok_any
