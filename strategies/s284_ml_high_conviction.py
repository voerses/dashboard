"""Strategy S284: ML High-Conviction Overlay (Long, Perp, 5x)
=============================================================
Same 8-token universe as s282, but ONLY fires on high-conviction signals
(p>=0.65 for stable, p>=0.68 for volatile tokens).
5x leverage because these signals have 2-3x higher forward returns.
Run ALONGSIDE s282 — the engine treats them as separate strategies, so
high-conviction bars get both s282 (3x) + s284 (5x) = effective ~8x.

Signal quality at p>=0.65:
  ETH: +5.33% avg fwd24h (vs 1.61% at p>=0.55)
  SOL: +5.68% (vs 1.86%)
  DOGE: +6.93% (vs 3.04%)
  XRP: +11.33% (vs 2.85%)
  ADA: +5.51% (vs 3.02%)
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    if token and token not in _WHITELIST:
        return StrategyResult(
            ...
            max_trade_pct=0.35, exchange='binance', name='s284_ml_high_conviction')
    ...
    # HIGH conviction threshold: 0.65 for stable, 0.68 for volatile
    threshold = 0.68 if token in _HIGH_VOL else 0.65
    ...
    return StrategyResult(
        ...
        market_type=MarketType.PERP, leverage=5.0,   # 5x leverage on high-conviction
        stop_mult=10.0,
        trail_mult=999.0,
        target_mult=999,
        no_stop_bars=18,
        min_hold=24, max_hold=24,
        edge=3.00,       # Higher edge for high-conviction signals
        exit_regimes=set(),
        breakeven_atr=999.0,
        max_trade_pct=0.35,
        conviction_score=conviction,
        exchange='binance', name='s284_ml_high_conviction')
