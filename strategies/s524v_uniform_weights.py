"""
S524v — Uniform Weights (no per-token IC optimization)
=================================================
...
BACKTEST CLI:
  /workspace/venv/bin/python v4/portfolio_backtest.py \
      --strategy s524v_uniform_weights --months 12 --capital 100000 \
...
def _load_daily_signals(symbol: str, ticker: str):
    ...
    # UNIFORM WEIGHTS: equal weight for all components, contrarian sign for all
    # This removes the per-token IC optimization to test true signal strength.
    w_oi = 1.0 / 3.0
    w_pos = 1.0 / 3.0
    w_flow = 1.0 / 3.0

    # Contrarian signs: negative OI (crowd is long → go short), etc.
    # Using the most common sign pattern from the config (contrarian for all 3)
    oi_sign = -1
    pos_sign = -1
    flow_sign = -1
...
def _compute_token_signal(ctx) -> StrategyResult:
    ...
    # Skip blacklisted tokens (config not needed for uniform weights, but
    # keep the gate to maintain same token universe for fair comparison)
    if ticker in TOKEN_BLACKLIST or ticker not in _token_configs:
        return StrategyResult(
            ...
            max_hold=720,
            edge=0.0,
            name='s524v_uniform_weights',
            breakeven_atr=BREAKEVEN_ATR,
        )

    # UNIFORM: fixed max hold for all tokens (median from config is ~720h = 30d)
    token_max_hold = 720
