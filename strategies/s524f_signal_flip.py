...
#       --strategy s524f_signal_flip --months 12 --capital 100000 \
#       --market perp --conviction-mode ranked \
#       --max-portfolio-positions 40 --concentration 0.30 --skip-wf \
#       --end-date 2026-04-05T16:00:00
#
S524f — Regime-Dependent Signal Flipping
==========================================

Based on s524b with REGIME-DEPENDENT SIGNAL FLIPPING:
- In BULL regime: flip contrarian tokens to momentum (composite * -1)
- In BEAR regime: keep contrarian (mean-reversion works in bears)
- Hypothesis: contrarian tokens fight the trend in bull markets (2024)

Also includes TOTAL2/TOTAL3 altcoin market cap signals:
...
if os.path.exists(_CONFIG_PATH):
    ...
    print(f"  [s524f] Loaded per-token configs for {len(_token_configs)} tokens")
else:
    print(f"  [s524f] WARNING: Config not found at {_CONFIG_PATH}")
...
def _load_total2_signals(idx_1h: pd.DatetimeIndex, btc_1h_series: pd.Series) -> dict:
    ...
    try:
        if not os.path.exists(_TOTAL2_PATH):
            print("  [s524f] WARNING: TOTAL2 data not found, using fallback")
        ...
        print(f"  [s524f] WARNING: TOTAL2 load failed ({exc}), using fallback")
        result = fallback
...
def _compute_reversal_regime(idx_1h: pd.DatetimeIndex, btc_1h: pd.Series) -> np.ndarray:
    ...
    try:
        ...
        print(f"  [s524f] WARNING: Reversal regime failed ({exc}), falling back to all-BULL")
...
def _load_daily_signals(symbol: str, ticker: str):
    ...
    try:
        ...
        print(f"  [s524f] WARNING: Failed to load {parquet_path}: {exc}")
        return
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    try:
        ...
        # TOTAL2 short gate: N=21d
        _t2_gate_path = os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
            'data', 'alternative', 'total2_total3.parquet',
        )
        if not hasattr(_get_composite_aligned, '_total2_close_cache'):
            _get_composite_aligned._total2_close_cache = None
        if _get_composite_aligned._total2_close_cache is None and os.path.exists(_t2_gate_path):
            _t2_df = pd.read_parquet(_t2_gate_path)
            if _t2_df.index.tz is not None:
                _t2_df.index = _t2_df.index.tz_localize(None)
            _get_composite_aligned._total2_close_cache = _t2_df['total2_close']
        _t2_close = _get_composite_aligned._total2_close_cache
        if _t2_close is not None:
            _t2_aligned = _t2_close.reindex(ctx.idx_1h.normalize(), method='ffill')
            _t2_aligned.index = ctx.idx_1h
            _t2_ret_Nd_h = (_t2_aligned / _t2_aligned.shift(21 * 24) - 1).values
            _t2_ret_Nd_h = np.nan_to_num(_t2_ret_Nd_h, nan=0)
            _short_ok = (_t2_ret_Nd_h < 0)
        else:
            _m_ret_45d_h = (btc_aligned / btc_aligned.shift(14 * 24) - 1).values
    ...
    # ==== REGIME-DEPENDENT SIGNAL FLIP ====
    # In BULL regime: flip contrarian tokens to momentum
    # The composite already has IC signs baked in. For contrarian tokens,
    # signs are negative -> composite is inverted. Flipping back = multiply by -1.
    # This makes contrarian tokens ride the trend in bull markets instead of fighting it.
    _token_direction = cfg.get("direction", "contrarian")
    if _token_direction == "contrarian":
        _bull_bars = ~_bear_regime
        # Flip adjusted_composite in BULL bars
        adjusted_composite = np.where(_bull_bars, -adjusted_composite, adjusted_composite)
        # Recompute abs_composite and entry signals after flip
        abs_composite = np.abs(np.nan_to_num(adjusted_composite, nan=0.0))
        if ENABLE_TOTAL2_LONG_THRESHOLD:
            _t2_sigs_pre = _load_total2_signals(ctx.idx_1h, btc_aligned)
            _t2_bear_pre = _t2_sigs_pre["total2_bear"]
