"""Strategy S282: ML Expanded Trend-Filtered (Long, Perp, 3x)
============================================================
s281 signal quality + expanded whitelist for more trades.
8 tokens: ETH/SOL/DOGE/XRP + ADA/LINK/SUI/HBAR (all ADV>$50M, >60% win).
Token-specific thresholds: p>=0.55 for stable, p>=0.58 for volatile.
DOWNTREND regime filter keeps DD <20%.
"""
import numpy as np
import pandas as pd
import joblib
...
# Expanded whitelist — 8 tokens with ADV>$50M and strong ML edge
_WHITELIST = {'ETH', 'SOL', 'DOGE', 'XRP', 'ADA', 'LINK', 'SUI', 'HBAR'}
# Higher confidence required for more volatile tokens
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    if token and token not in _WHITELIST:
        return StrategyResult(
            ...
            max_trade_pct=0.35, exchange='binance', name='s282_ml_expanded_filtered')
    ...
    return StrategyResult(
        ...
        exchange='binance', name='s282_ml_expanded_filtered')
