"""
S532 — Best Portfolio Strategy (bear-long stop 3.5 ATR + 24h Sharpe exit)
...
BACKTEST CLI:
  /workspace/venv/bin/python v4/portfolio_backtest.py \\
      --strategy s536_bounce_longs --months 12 --capital 100000 \\
...
def _compute_token_signal(ctx) -> StrategyResult:
    ...
    # BEAR BOUNCE: BTC > SMA50 for 7+ days during sustained bear
    _btc_vals = btc_aligned.values if hasattr(btc_aligned, 'values') else np.asarray(btc_aligned)
    _btc_s50 = pd.Series(_btc_vals).rolling(50*24, min_periods=20*24).mean().values
    _above_sma50 = np.nan_to_num(_btc_vals > _btc_s50, nan=False).astype(bool)
    _above_s50_count = np.zeros(n, dtype=int)
    for _j in range(1, min(n, len(_above_sma50))):
        _above_s50_count[_j] = _above_s50_count[_j-1] + 1 if _above_sma50[_j] else 0
    _bear_bounce = _sustained_bear & (_above_s50_count > 7 * 24)

    # Sustained bear long kill — BUT allow longs during bear bounce (relief rally = longs work)
    _kill_longs = entry & (direction == 1) & _sustained_bear & _post_halving & (~_bear_bounce)
    entry[_kill_longs] = False
    direction[_kill_longs] = 0
    conviction[~entry] = 0.0

    # Sustained bear leverage boost: shorts print hard in deep bear
    _regime_leverage[_sustained_bear & _post_halving] = 3.0
    # Reduce leverage during bear bounce (both longs and shorts at 1.5x)
    _regime_leverage[_bear_bounce] = 1.5
