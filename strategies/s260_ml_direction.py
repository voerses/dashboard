"""Strategy S260: ML Direction Predictor (Bidirectional, Perp, 2x)
================================================================
Uses ML model trained to predict DIRECTION of 3%+ moves in 24h.
- LONG when model predicts upward move with high confidence
- SHORT when model predicts downward move with high confidence
- Precision: 75-87% for LONG, 66-79% for SHORT (on unseen tokens)
"""
import numpy as np
import pandas as pd
import joblib
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    # Entry signals: only high-confidence direction predictions
    # Use moderate threshold for more trades but require regime filter
    entry_long = (long_prob >= 0.58) & (regime != CRISIS)
    entry_short = (short_prob >= 0.55) & (regime != CRISIS)
    ...
    # Conviction score: higher ML confidence = priority filling
    ...
    return StrategyResult(
        ...
        stop_mult=3.5, trail_mult=3.0, target_mult=999,
        no_stop_bars=24, min_hold=18, max_hold=720,
        edge=0.50, exit_regimes={CRISIS},
        breakeven_atr=0.8, max_trade_pct=0.15,
        conviction_score=conviction,
        exchange='binance', name='s260_ml_direction')
