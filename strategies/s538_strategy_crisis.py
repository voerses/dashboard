"""
S538 — CRISIS exit via exit_check_fn (engine regime read) (bear-long stop 3.5 ATR + 24h Sharpe exit)
...
BACKTEST CLI:
  /workspace/venv/bin/python v4/portfolio_backtest.py \\
      --strategy s538_strategy_crisis --months 12 --capital 100000 \\
...
def _sharpe_exit(pos, bar):
    # 1. CRISIS exit: read engine-computed regime. regime 0 = CRISIS.
    # Same as engine's RegimeExitHandler: wait regime_exit_min_bars (6), then exit on CRISIS.
...
def _compute_token_signal(ctx) -> StrategyResult:
    ...
    return StrategyResult(
        ...
        # CRISIS exit moved to exit_check_fn below  # tested: removing HURTS (+528% → +306%, DD -46% → -54%)
    )
