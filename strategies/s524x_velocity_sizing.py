"""
S524x — Velocity Sizing (size up on peaking, down on building) (bear-long stop 3.5 ATR + 24h Sharpe exit)
...
BACKTEST CLI:
  /workspace/venv/bin/python v4/portfolio_backtest.py \\
      --strategy s524x_velocity_sizing --months 12 --capital 100000 \\
...
def _load_daily_signals(symbol: str, ticker: str):
    ...
    # Z-score velocity: 7-day change (dz < 0 = peaking, good for contrarian)
...
def _compute_token_signal(ctx) -> StrategyResult:
    ...
    # Z-SCORE VELOCITY SIZING: peaking z-score → full size, building → reduced
    # Analysis: peaking (dz_7d < 0) has 44.8% WR / $2,975 avg (13.5x more PnL)
    # vs building (dz_7d > 0) at 32.4% WR / $186 avg
    _velocity = _get_velocity_aligned(symbol, ticker, ctx.idx_1h)
    _vel_peaking = _velocity < 0  # z-score declining = crowd unwinding = good entry
    _vel_building = _velocity > 0  # z-score increasing = crowd still piling on
    # Only apply in bear regime (where it adds +26pp in 2022, +89pp in 2025)
    # In bull/transition years it hurts — z-score oscillates without clear direction
    # Use _sustained_bear (>60d continuous bear) instead of _bear_regime
    # This targets 2022 and Q1-26 specifically, avoids brief 2025 dips
    _vel_bear_count = np.zeros(n, dtype=int)
    for _vi in range(1, n):
        _vel_bear_count[_vi] = _vel_bear_count[_vi-1] + 1 if _bear_regime[_vi] else 0
    _vel_sustained = _vel_bear_count > 60 * 24
    _size_mult[entry & _vel_peaking & _vel_sustained] *= 1.3
    _size_mult[entry & _vel_building & _vel_sustained] *= 0.5
