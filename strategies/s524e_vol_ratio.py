...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    try:
        ...
        # TOTAL2 21d gate: shorts when TOTAL2 21d return < 0 (direct alt market measure)
        # Falls back to BTC 45d if TOTAL2 unavailable
        ...
        if _get_composite_aligned._total2_close_cache is None and os.path.exists(_t2_gate_path):
            ...
            _get_composite_aligned._total2_close_cache = _t2g['total2_close']
        _t2c = _get_composite_aligned._total2_close_cache
        if _t2c is not None:
            _t2a = _t2c.reindex(ctx.idx_1h.normalize(), method='ffill')
            _t2a.index = ctx.idx_1h
            _t2r = (_t2a / _t2a.shift(21 * 24) - 1).values
            _short_ok = (np.nan_to_num(_t2r, nan=0) < 0)
        else:
            _m_ret_45d_h = (btc_aligned / btc_aligned.shift(45 * 24) - 1).values
            _short_ok = (np.nan_to_num(_m_ret_45d_h, nan=0) < 0)
    ...
    # VOL RATIO conviction scaler: T2/BTC vol ratio detects alt stress
    # High ratio (>1.3) = alts more volatile than BTC = alt stress → reduce longs, boost shorts
    # Low ratio (<0.9) = BTC leading, alts stable → alts will follow, boost longs
    try:
        _t2_close_vr = _get_composite_aligned._total2_close_cache if hasattr(_get_composite_aligned, '_total2_close_cache') and _get_composite_aligned._total2_close_cache is not None else None
        if _t2_close_vr is not None:
            _t2a_vr = _t2_close_vr.reindex(ctx.idx_1h.normalize(), method='ffill')
            _t2a_vr.index = ctx.idx_1h
            _t2_ret_d = _t2a_vr.pct_change(24)  # daily return on 1h bars
            _btc_ret_d = btc_aligned.pct_change(24)
            _t2_vol_30d = _t2_ret_d.rolling(30 * 24, min_periods=15 * 24).std().values
            _btc_vol_30d = _btc_ret_d.rolling(30 * 24, min_periods=15 * 24).std().values
            _vol_ratio = np.nan_to_num(_t2_vol_30d / (_btc_vol_30d + 1e-10), nan=1.0)

            # Alt stress: vol ratio > 1.3 → reduce long conviction 0.8x, boost short 1.15x
            _alt_stress = _vol_ratio > 1.3
            conviction[_lm & _alt_stress] *= 0.8
            conviction[_sm & _alt_stress] *= 1.15

            # BTC leading: vol ratio < 0.9 → boost long conviction 1.1x (alts will follow)
            _btc_leading = _vol_ratio < 0.9
            conviction[_lm & _btc_leading] *= 1.1
    except:
        pass
