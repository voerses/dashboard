"""Strategy S286: ML Expanded Trend-Filtered 5x Leverage (Long, Perp)
====================================================================
Same signal as s282 but 5x leverage. With s285 short hedging,
portfolio DD drops to -8.7%, leaving ~11% DD budget for higher leverage.
5x vs 3x should scale returns by ~1.67x on the long side.
"""
import numpy as np
import pandas as pd
import joblib
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    if token and token not in _WHITELIST:
        return StrategyResult(
            ...
            max_trade_pct=0.35, exchange='binance', name='s286_ml_expanded_5x')
    ...
    return StrategyResult(
        ...
        market_type=MarketType.PERP, leverage=5.0,   # 5x leverage
        stop_mult=10.0,
        trail_mult=999.0,
        target_mult=999,
        no_stop_bars=18,
        ...
        exchange='binance', name='s286_ml_expanded_5x')
