"""
S524u — Velocity + 24h Sharpe Exit (kill directionless trades early)
=================================================
...
BACKTEST CLI:
  /workspace/venv/bin/python v4/portfolio_backtest.py \
      --strategy s524u_velocity_exit --months 12 --capital 100000 \
...
# -- Velocity + 24h Sharpe exit --
# Analysis showed: winners establish direction by h8 (velocity > 0), losers don't.
# 24h Sharpe > -0.10 lifts WR from 38% to 44.5%.
# Velocity at h8: 43.7% WR (positive) vs 33.8% (negative) = 9.9pp split.
from v4.exit_handlers import ExitCheck

# Track per-position close prices for velocity/Sharpe computation
_pos_closes: dict = {}  # position_id -> list of close prices

VELOCITY_CHECK_HOUR = 8       # check velocity at this hour
VELOCITY_THRESHOLD = 0.0      # min directional return at check hour (0 = must be positive)
SHARPE_CHECK_HOUR = 24        # check 24h Sharpe at this hour
SHARPE_THRESHOLD = -0.50
# Bear-long tighter stop (proven +12pp in 2024)
STOP_MULT_BEAR_LONG = 3.5

def _velocity_sharpe_exit(pos, bar):
    """Kill trades that haven't established favorable direction by h8/h24.

    Two checks:
    1. At hour 8: if directional return is negative, exit (velocity check)
    2. At hour 24: if 24h Sharpe < -0.10, exit (consistency check)

    Uses _pos_closes dict to track price path per position.
    """
    pid = pos.position_id
    d = pos.direction
    ...
    # Clean up closed positions (avoid memory leak)
    # Can't detect closure here, but the list stays small (max_hold ~720 entries)

    held = bar.bars_held

    # Check 1: Velocity at h8 — DISABLED, mean-reversion trades are naturally underwater at h8
    # if held == VELOCITY_CHECK_HOUR: ...

    # Check 2: 24h Sharpe at hour 24
    if held == SHARPE_CHECK_HOUR and len(closes) >= SHARPE_CHECK_HOUR:
        import numpy as _np
        c = _np.array(closes[-SHARPE_CHECK_HOUR:])
        rets = _np.diff(c) / c[:-1] * d  # directional hourly returns
...
PORTFOLIO_CONFIG = {
    ...
    "max_positions": 50,
    "exit_check_fn": _velocity_sharpe_exit,
}
...
def _compute_token_signal(ctx) -> StrategyResult:
    ...
    # Regime-dependent stops (bear long 3.5 ATR: +12pp in 2024)
