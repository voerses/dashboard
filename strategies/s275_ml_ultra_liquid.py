"""Strategy S275: ML Direction on Ultra-Liquid Tokens (Long, Perp, 3x)
===================================================================
KEY INSIGHT: Raw edge is +1-8% but slippage on medium-cap tokens
eats it. On BTC/ETH/SOL (ADV >$1B), slippage is <10 bps total.
Focus: only trade ADV > $1B tokens. Use 3x leverage.
BTC: 278 signals, +1.13% avg. ETH: 133 signals, +3.19%.
SOL: 123 signals, +3.16%. XRP: 77 signals, +7.99%.
"""
import numpy as np
import pandas as pd
import joblib
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    # ULTRA-LIQUID FILTER: ADV > $1B
    # Only BTC, ETH, SOL, XRP, and a few others qualify
    ...
    adv_filter = rolling_dv > 1_000_000_000  # $1B daily volume
    ...
    return StrategyResult(
        ...
        exchange='binance', name='s275_ml_ultra_liquid')
