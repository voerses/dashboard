"""
S540 — BEST STRATEGY: Smart CRISIS (only exit losers in crisis) (bear-long stop 3.5 ATR + 24h Sharpe exit)
...
BACKTEST CLI:
  /workspace/venv/bin/python v4/portfolio_backtest.py \\
      --strategy s540_smart_crisis --months 12 --capital 100000 \\
...
def _sharpe_exit(pos, bar):
    # SMART CRISIS exit: only cut losers, ride winners through volatility
    if bar.bars_held > 6 and bar.regime == 0:
        _unreal = (bar.close - pos.entry_price) / max(pos.entry_price, 1e-10) * pos.direction
        if _unreal < 0:
...
def _compute_token_signal(ctx) -> StrategyResult:
    ...
    return StrategyResult(
        ...
        # exit_regimes replaced by exit_check_fn (CRISIS check in _sharpe_exit)
    )
