"""Strategy S292: ML 5x Aggressive (Long, Perp, 5x, cap_mult=2.0)
====================================================================
s286 with doubled position sizes via cap_multiplier=2.0.
cap_pct goes from 2-12% to 4-24% of equity.
Combined with s285 short hedge, DD should stay under 20%.

Math: s286+s285 = +148%, -9.3% DD. Doubling long positions:
  Long contribution ~doubles: 148% * (190K/313K) * 2 ≈ +180% from longs
  Short contribution stays: ~+60% from shorts
  Net DD: ~14-18% (within budget)
"""
import numpy as np
import pandas as pd
import joblib
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    if token and token not in _WHITELIST:
        return StrategyResult(
            ...
            cap_multiplier=1.85, max_trade_pct=0.50,
            exchange='binance', name='s300_ml_5x_edge3')
    ...
    return StrategyResult(
        ...
        exit_regimes=set(),
        breakeven_atr=999.0,
        cap_multiplier=1.85,      # 1.85x position size cap
        max_trade_pct=0.50,       # allow up to 50% of equity per trade
