"""
s105 MACD Squeeze Tuned — Micro-tuned s102 for DD target

s102 achieved: +400.0% return, MaxDD -22.6%, Calmar 5.45
Problem: DD 2.6pp above 20% target. Returns have 100pp of margin (400 vs 300).

Strategy: Trade some return margin for DD reduction via small parameter tweaks:
  1. LEVERAGE: 6.5x (from 7x) — 7% reduction in exposure
  2. GRACE PERIOD: 48h (from 72h) — trail activates sooner, limits unprotected DD
  3. BREAKEVEN RATCHET + VOL SCALING: Kept from s102
...
# ── Configuration ────────────────────────────────────────────────
LEVERAGE = 6.5                   # Slightly reduced (s98/s102 = 7.0)
WARMUP = 200
...
# Trade management
STOP_MULT = 99.0
TRAIL_MULT = 2.5                # Same as s98/s102
TARGET_MULT = 999.0
NO_STOP_BARS = 48               # Shorter grace (s98/s102 = 72h)
MIN_HOLD = 24
MAX_HOLD = 720
EDGE = 0.40
BREAKEVEN_ATR = 0.5
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    """MACD squeeze micro-tuned for DD control."""
