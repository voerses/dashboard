"""Strategy S271: ML Direction Long-Only, 1x Leverage, ADV-Filtered
================================================================
1x leverage: halves costs, limits catastrophic losses.
Raw edge +1.58% on high-ADV tokens. 1x cost ~0.6%. Net ~1.0%.
With enough trades, that compounds.
"""
import numpy as np
import pandas as pd
import joblib
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    return StrategyResult(
        ...
        market_type=MarketType.PERP, leverage=1.0,  # 1x leverage — halves all costs
        stop_mult=6.0,        # Wide stop for emergencies (6 ATR)
        trail_mult=999.0,     # No trailing stop
        target_mult=999,      # No target
        no_stop_bars=24,      # No stop during prediction horizon
        min_hold=20, max_hold=28,
        edge=0.80,            # Larger positions
        exit_regimes={CRISIS},
        breakeven_atr=999.0,
        max_trade_pct=0.20,
        conviction_score=conviction,
        exchange='binance', name='s271_ml_long_1x')
