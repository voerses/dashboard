"""
S524s — Conditional Exit (probability surface-driven stops)
=================================================
...
BACKTEST CLI:
  /workspace/venv/bin/python v4/portfolio_backtest.py \
      --strategy s524s_conditional_exit --months 12 --capital 100000 \
...
# -- Conditional exit rules (from probability surface analysis) --
# Regime-dependent initial stops: tighter where winners have low MAE
# Bear long winners: median MAE 4.4%, losers 9.4% → stop at 3.5 ATR (~5.6%)
# Bull long losers: 0-7% WR when <-10% at any hour → stop at 5.0 ATR (default)
# Bear/bull shorts: similar MAE profiles → keep default
STOP_MULT_BEAR_LONG = 3.5

# -- Custom exit function: probability surface checkpoint exits --
# Uses exit_check_fn (new engine hook) to check unrealized PnL at key hours.
# If unrealized PnL from entry is below the 25% WR threshold, exit the trade.
# Thresholds from research/exit_probability_surface.json:
#
# The key insight: check unrealized PnL FROM ENTRY (not from peak) at checkpoints.
# No existing handler does this — stops are from entry, trails are from peak.
from v4.exit_handlers import ExitCheck

def _conditional_exit_check(pos, bar):
    """Exit trades where P(winner) < 25% based on unrealized PnL at checkpoint hours."""
    held = bar.bars_held
    if held < 72:
        return None  # too early, let no_stop_bars protect
    ...
    # Checkpoints: (min_hours, max_hours, threshold_pct)
    # Below threshold = WR < 25%, exit the trade.
    # Thresholds are from the probability surface, rounded conservatively.
    # Only check at the appropriate window (don't re-trigger every bar).
    # Conservative: only exit deeply underwater trades (WR < 15%)
    checkpoints = [
        (120,  168, -10.0),   # hour 120-168: <-10% → 0-14% WR across all regimes
        (168,  336,  -7.0),   # hour 168-336: <-7% → 0-17% WR
        (336,  999,  -5.0),   # hour 336+: <-5% → 0-3% WR
    ]

    for min_h, max_h, threshold in checkpoints:
        if min_h <= held < max_h:
            if pnl_pct < threshold:
                return ExitCheck(should_exit=True, reason="conditional")
            break  # only one checkpoint applies
...
PORTFOLIO_CONFIG = {
    ...
    "max_positions": 50,
    "exit_check_fn": _conditional_exit_check,
}
...
def _compute_token_signal(ctx) -> StrategyResult:
...
def strategy(contexts: dict) -> dict:
    ...
    for token in sorted(contexts.keys()):
        ...
        try:
            sr = _compute_token_signal(ctx)
            if sr is not None and np.any(sr.entry_mask):
                token_results[token] = sr
        except Exception:
            continue
    ...
    # Step 2: Find day_change bars (all tokens share the same 1h index)
    # Use the first token's context to get the index
    sample_ctx = None
    for token in contexts:
        ctx_pair = contexts[token]
        if isinstance(ctx_pair, tuple):
            sample_ctx = ctx_pair[1] if ctx_pair[1] is not None else ctx_pair[0]
        else:
            sample_ctx = ctx_pair
    ...
    idx = sample_ctx.idx_1h
    n = len(idx)
    dates = idx.normalize()
    day_change_bars = [0]
    for i in range(1, n):
        if dates[i] != dates[i-1]:
            day_change_bars.append(i)
    ...
    for bar in day_change_bars:
        ...
        if len(candidates) <= MAX_ENTRIES_PER_BAR:
            continue  # few enough, keep all
        
        # Rank by conviction descending, keep top-N
        candidates.sort(key=lambda x: x[2], reverse=True)
        keep = set(c[0] for c in candidates[:MAX_ENTRIES_PER_BAR])
        
        # Suppress entries for tokens not in top-N
        for token, direction, conviction, b in candidates:
            if token not in keep:
                sr = token_results[token]
                sr.entry_mask[b] = False
                sr.direction[b] = 0
                if sr.conviction_score is not None and b < len(sr.conviction_score):
                    sr.conviction_score[b] = 0.0
