"""Strategy S312: ML V2 Long (Long, Perp, 5x, cap_mult=2.1)
====================================================================
Long-only strategy using V2 HistGBM model with market-context features.
No manual regime filter — the model handles regime awareness internally
via BTC returns, market regime, dispersion, breadth features.
"""
import numpy as np
import pandas as pd
import joblib
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    if token and token not in _WHITELIST:
        return StrategyResult(
            ...
            exchange='binance', name='s312_ml_v2_long')
    ...
    threshold = 0.72 if token in _HIGH_VOL else 0.70

    # No manual regime filter — model handles regime internally
    entry_mask = long_prob >= threshold
