...
#       --strategy s523n_fluid --months 12 --capital 100000 \
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    # s523n: NO binary short gate. Both directions always allowed.
    # Direction preference handled by FLUID conviction weighting below.
    long_signal = long_signal & day_change & rsi_long_window & _dilution_ok_long & _dilution_ok_any
    short_signal = short_signal & day_change & rsi_short_window & _dilution_ok_any
    ...
    # ==== s523n: FLUID conviction weighting ====
    # Compute a continuous regime score from -1 (strong bull) to +1 (strong bear)
    # Then map to per-direction conviction multipliers (0.0 to 1.5)
    try:
        # Component 1: BTC 90d return → normalized to [-1, +1]
        _r90 = (btc_aligned / btc_aligned.shift(90*24) - 1).values
        _r90_norm = np.clip(np.nan_to_num(-_r90, nan=0) / 0.30, -1, 1)  # -30% → +1, +30% → -1

        # Component 2: BTC monthly return trend (3mo rolling)
        _m3_vals = np.nan_to_num(_m_ret_1mo_h, nan=0)  # already hourly from v2 block
        _m3_norm = np.clip(-_m3_vals / 0.15, -1, 1)  # -15% month → +1, +15% → -1

        # Component 3: BTC vs SMA200d
        _sma200d = btc_aligned.rolling(200*24, min_periods=100*24).mean()
        _dist200 = ((btc_aligned - _sma200d) / _sma200d).values
        _dist200_norm = np.clip(np.nan_to_num(-_dist200, nan=0) / 0.20, -1, 1)  # -20% below → +1

        # Component 4: Momentum decay (30d vs 90d)
        _r30 = (btc_aligned / btc_aligned.shift(30*24) - 1).values
        _decay = np.nan_to_num(_r90, nan=0) - np.nan_to_num(_r30, nan=0)  # positive = decaying
        _decay_norm = np.clip(_decay / 0.15, -1, 1)

        # Weighted regime score: -1 (strong bull) to +1 (strong bear)
        _regime_score = (
            0.35 * _r90_norm +      # 90d return (strongest weight)
            0.25 * _m3_norm +       # monthly return trend
            0.25 * _dist200_norm +  # distance from SMA200d
            0.15 * _decay_norm      # momentum decay
        )
        # Smooth with 7-day rolling average to avoid daily noise
        _regime_smooth = pd.Series(_regime_score).rolling(7*24, min_periods=24).mean().values
        _regime_smooth = np.nan_to_num(_regime_smooth, nan=0)

        # Map to conviction multipliers (0.0 to 1.5)
        # Short conviction: 0 in strong bull → 1.5 in strong bear
        _short_conv = np.clip(0.75 + _regime_smooth * 0.75, 0.0, 1.5)
        # Long conviction: 1.5 in strong bull → 0.3 in strong bear (never 0 — longs can still work)
        _long_conv_fluid = np.clip(0.9 - _regime_smooth * 0.6, 0.3, 1.5)

        # Apply
        _lm = direction == 1
        _sm = direction == -1
        conviction[_lm] *= _long_conv_fluid[_lm]
        conviction[_sm] *= _short_conv[_sm]
    except Exception:
        # Fallback: v2 behavior (boost longs in trending-up)
        _lm = direction == 1
        conviction[_lm] *= _long_conv[_lm]
    # ==== end fluid conviction ====
