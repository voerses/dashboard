...
#       --strategy s523l_breadth_adaptive --months 12 --capital 100000 \
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    # s523k = s523j + dilution filter.
# s523j gives +74.5%, +386.3%, +593.8%, -32.2% (2022-2025).
# Dilution filter removes: longs on tokens with >50% supply remaining,
# ALL trades on tokens with >75% remaining. Research: this fixes 2025
# from -32% to ~+185% by removing high-dilution longs that face
# systematic selling pressure from token unlocks.
    ...
    try:
        ...
        # Combined short gate: breadth<30% OR 1mo_red
        # breadth<30% catches ALT BLEED (2025: alts bleed while BTC holds)
        # 1mo_red catches BEAR corrections (BTC red month)
        # Together: +60% more PnL than 1mo_red alone (research: $184K vs $115K)
        _short_ok_1mo = (np.nan_to_num(_m_ret_1mo_h, nan=0) < 0)

        # Compute alt breadth: % of alts above their 50-day SMA
        # alt breadth will be computed as per-bar array below
        try:
            _ohlcv_dir = os.path.join(
                os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                'data', 'perp', 'binance', '1h_ohlcv',
            )
            if not hasattr(_get_composite_aligned, '_breadth_cache'):
                _get_composite_aligned._breadth_cache = None
            if _get_composite_aligned._breadth_cache is None:
                import pandas as _pd_br
                # Load top 80 alts by file size (proxy for data length)
                _alt_files = sorted(
                    [(f, os.path.getsize(os.path.join(_ohlcv_dir, f)))
                     for f in os.listdir(_ohlcv_dir)
                     if f.endswith('.csv') and not f.startswith('BTC')],
                    key=lambda x: -x[1]
                )[:80]
                _alt_daily = {}
                for _af, _ in _alt_files:
                    _tok = _af.replace('_perp_1h.csv', '')
                    try:
                        _adf = _pd_br.read_csv(os.path.join(_ohlcv_dir, _af),
                                               parse_dates=['datetime']).set_index('datetime')
                        if _adf.index.tz:
                            _adf.index = _adf.index.tz_convert(None)
                        _alt_daily[_tok] = _adf['close'].astype(float).resample('D').last().dropna()
                    except Exception:
                        pass
                # Compute daily breadth
                _all_dates = _pd_br.date_range('2021-01-01', '2026-12-31', freq='D')
                _breadth_series = _pd_br.Series(dtype=float, index=_all_dates)
                for _d in _all_dates:
                    _above = 0
                    _total = 0
                    for _tok, _s in _alt_daily.items():
                        _ix = _s.index.get_indexer([_d], method='pad')[0]
                        if _ix < 50:
                            continue
                        _p = float(_s.iloc[_ix])
                        _sma = float(_s.iloc[max(0,_ix-50):_ix+1].mean())
                        _total += 1
                        if _p > _sma:
                            _above += 1
                    if _total > 10:
                        _breadth_series[_d] = _above / _total * 100
                _get_composite_aligned._breadth_cache = _breadth_series.dropna()
            _br = _get_composite_aligned._breadth_cache
            # Forward-fill daily breadth to hourly index (PER-BAR, not scalar!)
            _breadth_h = _br.reindex(ctx.idx_1h, method='ffill').values
            _breadth_h = np.nan_to_num(_breadth_h, nan=50.0)
        except Exception:
            _breadth_h = np.full(n, 50.0)

        # Combined gate: shorts allowed when alts broadly weak OR BTC red month
        # PER-BAR: each bar gets the breadth at THAT date, not a global scalar
        _alt_bleed = _breadth_h < 30.0
        _short_ok = _alt_bleed | _short_ok_1mo
    ...
    # ==== DILUTION FILTER ====
    # No longs on tokens >50% remaining (unlock pressure kills longs)
    # No trades at all on tokens >75% remaining (chaotic, bad for both directions)
    _dilution_ok_long = np.ones(n, dtype=bool)
    _dilution_ok_any = np.ones(n, dtype=bool)
    try:
        _dilution_path = os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
        ...
        if _get_composite_aligned._dilution_cache is None:
            import pandas as _pd_dil
            _dil_df = _pd_dil.read_parquet(_dilution_path)
            _dil_dict = {}
            for _, _row in _dil_df.iterrows():
                _key = (_row['token'], _row['date'].strftime('%Y-%m'))
                _dil_dict[_key] = float(_row['remaining_pct'])
            _get_composite_aligned._dilution_cache = _dil_dict
        _dil_dict = _get_composite_aligned._dilution_cache

        _tok_sym = ticker.replace('USDT', '') if ticker.endswith('USDT') else ticker
        _latest_remaining = 0.0
        for _m_offset in range(0, 60):
            _check_date = (ctx.idx_1h[-1] - pd.Timedelta(days=_m_offset * 30)).strftime('%Y-%m')
            if (_tok_sym, _check_date) in _dil_dict:
                _latest_remaining = _dil_dict[(_tok_sym, _check_date)]
                break

        if _latest_remaining > 75:
            _dilution_ok_any[:] = False
        elif _latest_remaining > 50:
            _dilution_ok_long[:] = False
    except Exception:
        pass
    # ==== end dilution filter ====

    # v2 proven + dilution filter
    long_signal = long_signal & day_change & rsi_long_window & _dilution_ok_long & _dilution_ok_any
    short_signal = short_signal & day_change & rsi_short_window & _short_ok & _dilution_ok_any
