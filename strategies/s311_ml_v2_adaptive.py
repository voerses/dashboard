"""Strategy S311: ML V2 Adaptive (Long+Short, Perp, market-context-aware)
====================================================================
Uses the V2 ML model (HistGBM + 41 features including market context).
The model itself is market-aware: it sees BTC returns, dispersion, regime,
breadth, and token-BTC correlation. No manual regime filters needed.

Direction:
  model prob(long) >= threshold  -> go long, 5x leverage
  model prob(short) >= threshold -> go short, 4x leverage
  Neither -> no entry

Per-bar: direction, leverage, cap_multiplier set based on model signal.
"""
import numpy as np
import pandas as pd
import joblib
...
# ==================== Precompute market context (once at import) ====================
...
def _build_market_context():
    """Precompute shared market-level features."""
    ...
    regime_daily[uptrend] = 2; regime_daily[downtrend] = 4
    ...
    # Cross-asset breadth + dispersion (top 30 tokens)
...
# Build market context at import time
...
def _compute_features(close, high, low, volume, funding, idx_1h):
    """Compute 41 features (34 base + 7 market context)."""
    n = len(close)
    ...
    # Token-BTC correlation (168h rolling)
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    if token and token not in _WHITELIST:
        return StrategyResult(
            ...
            market_type=MarketType.PERP, leverage=np.ones(n) * 5.0,
            ...
            cap_multiplier=1.05, size_multiplier=np.ones(n),
            max_trade_pct=0.50,
            exchange='binance', name='s311_ml_v2_adaptive')
    ...
    long_threshold = 0.58 if token in _HIGH_VOL else 0.55
    short_threshold = 0.60 if token in _HIGH_VOL else 0.60

    # --- Per-bar adaptive signals ---
    entry_mask = np.zeros(n, dtype=bool)
    direction = np.zeros(n, dtype=np.int8)
    leverage = np.ones(n, dtype=np.float64) * 5.0
    ...
    leverage[long_signal] = 5.0
    cap_mult[long_signal] = 1.05  # 2-strat effective: weight=0.5 * cm=2.1 = 1.05

    # Short signals (higher threshold for quality)
    short_signal = short_prob >= short_threshold
    # Where both fire, short takes precedence only if short_prob > long_prob
    both = long_signal & short_signal
    short_wins = both & (short_prob > long_prob)
    short_only = short_signal & ~long_signal
    short_active = short_only | short_wins

    entry_mask[short_active] = True
    direction[short_active] = -1
    leverage[short_active] = 4.0
    cap_mult[short_active] = 0.65  # 2-strat effective: weight=0.5 * cm=1.3 = 0.65

    # Warmup + liquidity filter
