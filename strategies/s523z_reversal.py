...
#
S523z — Reversal State Machine Regime Detection
=================================================

Based on s523v with the hardcoded year-based bear/bull regime replaced by a
causal state machine that detects cycle tops and bottoms using:

BEAR LOW -> BULL: score-based (rally from low, RSI, alt breadth, momentum)
  Requires above SMA200 or rally > 80% to prevent false bear market bounces.

BULL HIGH -> BEAR: requires ATH drawdown < -25% AND near-ATH within 120 days
  Plus scoring (alt breadth collapse, momentum decline, SMA200 breakdown).
  The near-ATH requirement prevents false triggers from stale historical ATHs.

Short gating: BULL regime uses G45 gate (45d return < 0).
              BEAR regime has shorts ungated.
Deep bear: DB-10% long suppression preserved.
All other logic identical to s523v.
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    # ==== HALVING CYCLE + REVERSAL STATE MACHINE ====
    # Short gating: halving cycle prior (post-halving years = shorts ungated)
    ...
    # The 1.2x bull-long boost helps during sustained bull markets (2023, 2025 recovery).
    # Suppress boost in August: historically choppy month with corrections (2024 Aug, 2025 Aug).
    # Boosted longs get stopped out during these corrections; removing boost preserves capital.
