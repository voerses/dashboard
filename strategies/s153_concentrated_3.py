"""
s151 Volatile Token Funding L/S — V4 Portfolio Strategy (Class B)

The s85 funding L/S alpha came from volatile tokens. Instead of filtering them out
(which killed the signal), ONLY trade tokens with high funding dispersion.

Same cross-sectional L/S as s85 but restricted universe:
- Only tokens with funding std > 0.0002 (high dispersion)
- ADV > $5M (tradeable but includes smaller tokens)
- 24h funding window (fast signal)
- Dollar-neutral: 5L + 5S
- 1x leverage (no leverage on volatile tokens)
- Weekly rebalance
...
MIN_FUNDING_DISP = 0.0002     # Only trade high-dispersion tokens
MIN_ELIGIBLE = 12
LEVERAGE = 1.0
WARMUP = 250
...
def strategy(contexts: dict) -> dict:
    """Cross-sectional funding L/S on volatile tokens only."""
    token_data = {}
