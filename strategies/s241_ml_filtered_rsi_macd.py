"""
Strategy S241: ML-Filtered RSI-MACD (Bidirectional, Perp, 2x)
==============================================================
Uses ML model as a VOLATILITY FILTER: only enter RSI-MACD signals
when ML predicts a big move is coming (either direction).
Combines TA for direction with ML for timing.
"""
import numpy as np
import pandas as pd
import joblib
...
_model = joblib.load(os.path.join(_MODEL_DIR, 'ml_rf_model.joblib'))
_scaler = joblib.load(os.path.join(_MODEL_DIR, 'ml_rf_scaler.joblib'))
_feat_names = joblib.load(os.path.join(_MODEL_DIR, 'ml_rf_features.joblib'))
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    # TA signals (proven RSI-MACD + BB extreme)
    rsi = ctx.ind_1h['rsi']
    ...
    # RSI-MACD cross (proven signal)
    ta_long = ((rsi < 30) & (rsi > rsi_prev)
               & (macd > macd_sig) & (macd_prev <= ms_prev))
    ta_short = ((rsi > 70) & (rsi < rsi_prev)
                & (macd < macd_sig) & (macd_prev >= ms_prev))

    # BB extreme (supplementary)
    bb_long = (bb_pct < 0.03) & (rsi < 25) & (rsi > rsi_prev)
    bb_short = (bb_pct > 0.97) & (rsi > 75) & (rsi < rsi_prev)

    # Any TA signal for direction
    entry_long_ta = ta_long | bb_long
    entry_short_ta = ta_short | bb_short

    # ML volatility prediction: compute features
    ...
    # ML says "big move coming" = max(long_prob, short_prob) > 0.4
    ...
    ml_big_move = np.maximum(long_prob, short_prob) > 0.35

    # Combined: TA signal + ML volatility filter
    entry_long = entry_long_ta & ml_big_move & (regime != CRISIS)
    entry_short = entry_short_ta & ml_big_move & (regime != CRISIS)

    # Also: ML-only high confidence (p >= 0.6) without TA filter
    # These are rare but high quality
    ml_strong_long = (long_prob >= 0.60) & (rsi < 40) & (regime != CRISIS)
    ml_strong_short = (short_prob >= 0.55) & (rsi > 60) & (regime != CRISIS)

    entry_long = entry_long | ml_strong_long
    entry_short = entry_short | ml_strong_short
    ...
    return StrategyResult(
        ...
        exchange='binance', name='s241_ml_filtered_rsi_macd')
