...
#       --strategy s523l_bear_adaptive --months 12 --capital 100000 \
...
# systematic selling pressure from token unlocks.
    ...
    try:
        ...
        # 1mo_red short gate + bear-mode override
        _short_ok_1mo = (np.nan_to_num(_m_ret_1mo_h, nan=0) < 0)

        # Bear-mode detection: BTC 3mo return < -10% AND below daily SMA200
        # When detected → disable 1mo_red gate, allow ALL shorts
        # Research: detectable by April 2025, enables +329% short-only performance
        _btc_3mo_ret = btc_aligned / btc_aligned.shift(90 * 24) - 1
        _btc_sma200d = btc_aligned.rolling(200 * 24, min_periods=100 * 24).mean()
        _bear_mode = ((_btc_3mo_ret < -0.10) & (btc_aligned < _btc_sma200d)).values
        _bear_mode = np.nan_to_num(_bear_mode, nan=False).astype(bool)

        # In bear mode: ALL shorts allowed. In bull mode: 1mo_red gate.
        _short_ok = np.where(_bear_mode, True, _short_ok_1mo)
    ...
    # In bear mode: skip RSI for shorts (research: RSI blocks valid short entries in 2025)
    try:
        _short_rsi_gate = np.where(_bear_mode, True, rsi_short_window)
    except Exception:
        _short_rsi_gate = rsi_short_window
    short_signal = short_signal & day_change & _short_rsi_gate & _short_ok & _dilution_ok_any
