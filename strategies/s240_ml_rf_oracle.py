"""
Strategy S240: ML RandomForest Oracle (Bidirectional, Perp, 2x Leverage)
=========================================================================
Uses trained RandomForest to predict high-probability entries.
Model trained on 50 tokens, 15 months, labels: >5% forward move in 48h.
Walk-forward validated. 34 features.
"""
import numpy as np
import pandas as pd
import joblib
...
def _compute_features(close, high, low, volume, funding=None):
    ...
    cr=high-low
    uw=high-np.maximum(close,pc); lw=np.minimum(close,pc)-low
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    # Build feature matrix in sorted order matching training
    ...
    entry_long = (long_prob >= _LONG_THR) & (regime != CRISIS)
    entry_short = (short_prob >= _SHORT_THR) & (regime != CRISIS)
    ...
    # Conviction score for sizing priority
