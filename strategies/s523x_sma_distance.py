...
#       --strategy s523v_g45_db10 --months 12 --capital 100000 \
#       --market perp --conviction-mode ranked \
#       --max-portfolio-positions 40 --concentration 0.30 --skip-wf \
#       --end-date 2026-04-05T16:00:00
#
S523x — SMA50 Distance-Based Regime Detection
================================================

Evolved from s523v. Replaces hardcoded bear year list with SMA50 DISTANCE:
- Distance < -15%: BEAR → shorts ungated, deep bear at -10%
- Distance > +15%: BULL → shorts fully gated, deep bear at -3%
- -15% to +15%: TRANSITION → shorts gated by 45d return, deep bear at -5%

No hardcoded years. Fully causal. Works for any market cycle.
Distance captures regime CONVICTION, not just direction.
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    try:
        ...
        # G45 short gate (used in TRANSITION zone only)
        _m_ret_45d_h = (btc_aligned / btc_aligned.shift(45 * 24) - 1).values
        _m_ret_45d_h = np.nan_to_num(_m_ret_45d_h, nan=0)
        _short_ok_45d = (_m_ret_45d_h < 0)

        # ---- SMA50 DISTANCE-BASED REGIME ----
        # Distance = (price - SMA50) / SMA50 as percentage
        _sma50_dist = ((btc_aligned - _w_sma50) / _w_sma50).values
        _sma50_dist = np.nan_to_num(_sma50_dist, nan=0)

        # 3-zone classification:
        _is_bear = _sma50_dist < -0.15      # deep below SMA50 → confirmed bear
        _is_bull = _sma50_dist > 0.15       # well above SMA50 → confirmed bull
        _is_transition = ~_is_bear & ~_is_bull  # near SMA50 → uncertain

        # Short gate: BEAR=ungated, BULL=fully gated, TRANSITION=45d return gate
        _short_ok = np.where(
            _is_bear, True,
            np.where(_is_bull, False, _short_ok_45d)
        )

        # Conviction boost for trending-up in bull regime
    ...
    # SMA50 distance-based regime (no hardcoded years)
    # _short_ok already computed above: BEAR=ungated, BULL=gated, TRANSITION=45d

    long_signal=long_signal&day_change&rsi_long_window&_dol&_doa
    short_signal=short_signal&day_change&rsi_short_window&_short_ok&_doa

    # Distance-adaptive deep bear long suppression:
    # BEAR (dist < -15%): block longs when 30d return < -10%
    # TRANSITION (-15% to +15%): block longs when 30d return < -5%
    # BULL (dist > +15%): block longs when 30d return < -3%
    try:
        _db_thresh = np.where(
            _is_bear, -0.10,
            np.where(_is_bull, -0.03, -0.05)
        )
        _deep_bear = np.nan_to_num(_m_ret_1mo_h, nan=0) < _db_thresh
