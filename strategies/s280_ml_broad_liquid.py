"""Strategy S280: ML Broad Liquid (Long, Perp, 3x)
==================================================
EXPANDED whitelist: 12 tokens with ADV > $50M AND strong ML edge.
Key improvement over s277: 3x more tokens = 3x more trades.
Token selection: avg 24h return > +1.5%, win rate > 55%, trades > 50.
Expected ~700 trades vs 250 for s277 (4-token whitelist).
"""
import numpy as np
import pandas as pd
import joblib
...
# Broad liquid whitelist — ADV > $50M, strong ML edge
# ETH: +1.61% avg, 65% win, $7.1B ADV
# SOL: +1.93% avg, 68% win, $2.7B ADV
# DOGE: +3.08% avg, 73% win, $1.05B ADV
# XRP: +2.72% avg, 73% win, $1.63B ADV
# ADA: +3.02% avg, 72% win, $417M ADV
# LINK: +2.67% avg, 69% win, $286M ADV
# SUI: +2.14% avg, 62% win, $629M ADV
# HBAR: +3.38% avg, 73% win, $96M ADV
# ONDO: +2.99% avg, 70% win, $105M ADV
# TAO: +2.35% avg, 60% win, $124M ADV
# RENDER: +2.01% avg, 58% win, $38M ADV (borderline but strong signal)
# FARTCOIN: +1.27% avg, 53% win, $310M ADV (many trades)
# Only ADV > $200M with strong edge (win rate > 60%, avg return > +2%)
_WHITELIST = {'ETH', 'SOL', 'DOGE', 'XRP', 'ADA', 'LINK', 'SUI'}
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    if token and token not in _WHITELIST:
        return StrategyResult(
            ...
            max_trade_pct=0.35, exchange='binance', name='s280_ml_broad_liquid')
    ...
    entry_long = (long_prob >= 0.55) & (regime != CRISIS)
    ...
    return StrategyResult(
        ...
        market_type=MarketType.PERP, leverage=3.0,
        stop_mult=10.0,       # Catastrophic stop only
        trail_mult=999.0,
        target_mult=999,
        no_stop_bars=18,
        ...
        exchange='binance', name='s280_ml_broad_liquid')
