"""Strategy S276: ML Mega-Liquid Concentrated (Long, Perp, 3x)
================================================================
KEY INSIGHT: Ultra-liquid tokens (BTC/ETH/SOL/DOGE/XRP) have:
  - Strong ML edge: +0.4% to +3.1% raw 24h returns at p>=0.55
  - Minimal slippage (<10 bps) due to $1B+ ADV
  - Cost < 0.3% total per trade
IMPROVEMENT: Lower threshold (0.55 vs 0.60), bigger positions,
hardcoded whitelist (no buggy ADV filter letting in ZEC).
Expected: 395 trades at ~$1,700 net each = 334% return.
"""
import numpy as np
import pandas as pd
import joblib
...
# Hardcoded ultra-liquid whitelist — no ADV filter bugs
_WHITELIST = {'BTC', 'ETH', 'SOL', 'DOGE', 'XRP'}
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    # WHITELIST CHECK: only trade ultra-liquid tokens
    ...
    if token and token not in _WHITELIST:
        ...
        return StrategyResult(
            ...
            stop_mult=999.0, trail_mult=999.0, target_mult=999,
            no_stop_bars=999, min_hold=24, max_hold=24,
            edge=2.00, exit_regimes=set(), breakeven_atr=999.0,
            max_trade_pct=0.35, exchange='binance', name='s276_ml_mega_liquid')
    ...
    # Lower threshold for more signals — 0.55 vs 0.60
    ...
    return StrategyResult(
        ...
        market_type=MarketType.PERP, leverage=3.0,
        stop_mult=999.0,      # No stops
        trail_mult=999.0,
        target_mult=999,
        no_stop_bars=999,
        min_hold=24, max_hold=24,
        edge=2.00,            # Aggressive sizing — use more capital
        exit_regimes=set(),
        breakeven_atr=999.0,
        max_trade_pct=0.35,   # 35% of capital per trade
        conviction_score=conviction,
        exchange='binance', name='s276_ml_mega_liquid')
