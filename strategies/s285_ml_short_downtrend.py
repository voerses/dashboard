"""Strategy S285: ML Short During Downtrend (Short, Perp, 3x)
=============================================================
Opposite of s282: SHORT signals during DOWNTREND regime only.
The ML model's short_prob (class=-1) predicts price drops.
By only shorting during confirmed DOWNTREND, we:
1. Capture profit when longs are sidelined
2. Hedge portfolio DD during drawdown periods
3. Earn positive funding (shorts collect when funding > 0)

Short signal analysis (last 12mo):
  ETH: -0.80% avg fwd24h at short_prob>=0.55 (1520 signals)
  SOL: -1.23% (1165 signals)
  ADA: -1.80% (1063 signals)
  LINK: -0.96% (1124 signals)

At 3x leverage, -1.2% → +3.6% per short trade (shorts profit on decline).
"""
import numpy as np
import pandas as pd
import joblib
...
# Wider whitelist for shorts — focus on liquid tokens that drop hardest
...
def strategy(ctx: StrategyContext) -> StrategyResult:
    ...
    if token and token not in _WHITELIST:
        return StrategyResult(
            ...
            max_trade_pct=0.25, exchange='binance', name='s285_ml_short_downtrend')
    ...
    # Short threshold: higher for volatile tokens
    threshold = 0.60 if token in _HIGH_VOL else 0.55

    # ONLY short during DOWNTREND regime (when longs are sidelined)
    entry_short = (short_prob >= threshold) & (regime == DOWNTREND)

    entry_mask = entry_short
    direction = -np.ones(n, dtype=np.int8)  # SHORT direction
    ...
    return StrategyResult(
        ...
        exchange='binance', name='s285_ml_short_downtrend')
