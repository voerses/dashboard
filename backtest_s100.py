#!/usr/bin/env python3
"""Quick backtest for s100 dispersion momentum strategy."""
...
def run_backtest(months=12, capital=200_000):
    ...
    all_signals = {
        's100': precompute_strategy_signals(spec, tokens, config, months, end_date=data_end)
    }
    print(f"  Signals computed in {time.time()-t0:.1f}s")
    print(f"  Tokens with signals: {len(all_signals['s100'])}")

    strategy_specs = {'s100': spec}
    ...
    annual_ret = metrics.annualized_return * 100 if hasattr(metrics, 'annualized_return') else total_ret

    print(f"\n{'='*60}")
    print(f"  S100 DISPERSION MOMENTUM BACKTEST RESULTS")
    print(f"{'='*60}")
    ...
    print(f"  Annual Return:     {metrics.annualized_return_pct:>+8.1f}%")
    print(f"{'='*60}")

    # Target check
    print(f"\n  TARGET CHECK:")
    print(f"  Annual Return >= 300%:  {'PASS' if total_ret >= 300 else 'FAIL'} ({total_ret:+.1f}%)")
...
if __name__ == "__main__":
    ...
    run_backtest(args.months, args.capital)
