"""
s503 BB Breakout Hourly — Forward 1m Entry Baseline
=====================================================

Runs s503 (forward-only 1m entry after 4H BB signal) across three windows:
  - L12M (12 months)
  - L6M  (6 months)
  - L3M  (3 months)

Config: pos=50, lev=1.25, hold=4, edge=0.50, conviction=ranked, entry_resolution=1
        capital=100k, exchange=binance, skip_walk_forward=True

Usage:
    /workspace/venv/bin/python research/s503_baseline.py
"""
...
def run_backtest(tokens, months=12, label="default"):
    """Run s503 backtest with given parameters."""
    ...
    spec = StrategySpec(
        ...
        max_positions=50,
        entry_resolution=1,  # Forward 1m entry after 4H signal
    )

    print(f"\n{'='*70}")
    print(f"  Running: {label}")
    print(f"  entry_resolution=1 (forward 1m), months={months}")
    print(f"{'='*70}")
    ...
    state = simulate_portfolio({'s503': signals}, {'s503': spec}, config)
...
def print_metrics(m, header=""):
    ...
    print(f"  Ann. Return: {m['total_return'] / max(m['months'] / 12, 0.01):+.1f}% (annualized)")
...
def main():
    print("=" * 70)
    print("  s503 BB Breakout Hourly — Baseline Backtest")
    print("  entry_resolution=0, pos=50, lev=1.25, conviction=ranked")
    print("=" * 70)
    ...
    # Window 1: 12 months
    # ======================================================================
    m, state = run_backtest(all_tokens, months=12, label="s503_L12M")
    print_metrics(m, "s503 Hourly BB Breakout (12M)")
    ...
    # Window 2: 6 months
    # ======================================================================
    m, state = run_backtest(all_tokens, months=6, label="s503_L6M")
    print_metrics(m, "s503 Hourly BB Breakout (6M)")
    if state is not None:
        monthly_breakdown(state, 100_000)
    results['s503_L6M'] = m
    ...
    # Window 3: 3 months
    # ======================================================================
    m, state = run_backtest(all_tokens, months=3, label="s503_L3M")
    print_metrics(m, "s503 Hourly BB Breakout (3M)")
    if state is not None:
        monthly_breakdown(state, 100_000)
    results['s503_L3M'] = m
    ...
    # ======================================================================
    print("\n" + "=" * 70)
    print("  SUMMARY — s503 BB Breakout Hourly Baseline")
    print("=" * 70)
    print(f"\n{'Label':<16} {'Return':>9} {'Ann.Ret':>9} {'MaxDD':>7} {'Sharpe':>7} "
          f"{'Sortino':>8} {'Calmar':>7} {'PF':>6} {'WR':>6} {'Trades':>7} {'Fees':>10} {'Funding':>10}")
    print("-" * 110)
    for label, m in results.items():
        if m is None:
            print(f"  {label:<14} {'FAILED':>9}")
            continue
        ann_ret = m['total_return'] / max(m['months'] / 12, 0.01)
        print(f"  {label:<14} {m['total_return']:>+8.1f}% {ann_ret:>+8.1f}% {m['max_dd']:>6.1f}% "
              f"{m['sharpe']:>7.2f} {m['sortino']:>8.2f} {m['calmar']:>7.2f} "
              f"{m['pf']:>6.2f} {m['win_rate']:>5.1f}% {m.get('n_trades', 0):>7d} "
              f"${m['total_fees']:>9,.0f} ${m['total_funding']:>9,.0f}")
    ...
    with open('results/v4/s503_baseline.json', 'w') as f:
        json.dump(output, f, indent=2, default=str)
    print(f"\nResults saved to results/v4/s503_baseline.json")
