...
from v4.simulator import SimulationState, _process_margin_calls, _process_exits
...
# ===================================================================
# Test: margin call on a partially-closed position
# ===================================================================

class TestMarginCallAfterPartialClose:
    """Margin call should work on positions with reduced margin after partial TP."""

    def test_margin_call_on_partially_closed_position(self):
        """A position with halved margin/quantity from partial close can still be margin-called."""
        config = _default_config(capital=100_000.0)
        state = SimulationState(initial_capital=100_000.0)

        # Create a position and simulate partial close (halve margin and quantity)
        pos = _make_perp_position(
            margin_usd=50_000.0, cumulative_funding=3000.0,
            entry_price=100.0, leverage=1.0, direction=1,
        )
        # Simulate partial close: halve margin and quantity
        pos.margin_usd = 25_000.0
        pos.quantity = pos.quantity / 2
        pos.partial_closed = True

        state.position_manager.open_position(pos)
        state._entry_fees_by_pos[pos.position_id] = 5.0
        # Push funding high enough to breach threshold
        state.total_funding = 80_000.0
        # equity = 100K - 80K = 20K, locked = 25K, free = -5K
        # threshold = -max(20K * 0.05, 1) = -1K, free=-5K < -1K → triggers
        ...
        # Verify the partially-closed position's reduced margin was used
        assert trade.margin_usd == 25_000.0
