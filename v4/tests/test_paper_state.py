...
Tests verify:
  ...
  - Position serialization (including numpy arrays like trail_schedule, exit_regimes set)
...
def _make_test_position(
    ...
    return Position(
        ...
        min_hold=12,
        max_hold=720,
        exit_regimes=exit_regimes if exit_regimes is not None else {4},
        convex_exit=False,
...
class TestPositionSerialization:
    ...
    def test_exit_regimes_set_preserved(self):
        """exit_regimes (a set) must survive serialization."""
        state, tick_counter = _make_test_state()

        json_data = serialize_state(state, tick_counter, "2026-03-09T20:00:00Z")
        restored_state, _ = deserialize_state(json_data)

        # First position has exit_regimes={4}
        pos0 = restored_state.position_manager.open_positions[0]
        assert isinstance(pos0.exit_regimes, set)
        assert pos0.exit_regimes == {4}

        # Second position has exit_regimes={2, 3}
        pos1 = restored_state.position_manager.open_positions[1]
        assert isinstance(pos1.exit_regimes, set)
        assert pos1.exit_regimes == {2, 3}
