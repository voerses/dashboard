...
def process_minute_exits(
    ...
    for pos in list(state.position_manager.open_positions):
        ...
        for i in range(len(m_high)):
            ...
            # Partial profit-taking (backtest-specific: needs state + _partial_close_position)
            if (pos.partial_tp_atr > 0.0
                    and not pos.partial_closed
                    and bars_held >= pos.no_stop_bars):
                if d == 1:
                    profit_atr = (h_i - pos.entry_price) / max(cur_atr, 1e-10)
                else:
                    profit_atr = (pos.entry_price - l_i) / max(cur_atr, 1e-10)
                if profit_atr >= pos.partial_tp_atr:
                    _partial_close_position(state, pos, global_bar, c_i, adv_val, config)
...
def _update_trail_minute(pos: Position, cur_atr: float, bars_held: int) -> None:
    ...
    Mirrors sentinel _update_trail guard conditions:
