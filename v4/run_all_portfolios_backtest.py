...
from v4.simulator import build_unified_index, SimulationState, _process_exits, _process_entries, _close_all_remaining
...
def simulate_with_dynamic_weights(all_signals, strategy_specs, config, allocator=None):
    ...
    for global_bar in range(n_bars):
        ...
        if state.free_capital < -max(state.portfolio_equity * 0.05, 1.0):
            raise RuntimeError(f"Free capital invariant violated at bar {global_bar}")
