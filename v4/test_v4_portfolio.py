"""Comprehensive acceptance tests for the V4 Portfolio Backtest Engine.

Tests cover:
  - Unit tests: config defaults, position management, sizing, walk-forward masking,
  ...
  - Edge cases: empty signals, single bar, all entries masked, NaN ATR
...
def _make_token_signals(
    ...
    return TokenSignals(
        ...
        exit_regimes=exit_regimes if exit_regimes is not None else set(),
        stop_mult=stop_arr,
...
def _default_config(**overrides) -> PortfolioConfig:
    ...
    defaults = dict(
        ...
        impact_coeff=0.03,
        seed=42,
        train_bars=0,       # disable walk-forward mask by default in tests
        recal_bars=99999,
        purge_bars=0,
    )
...
class TestConfigDefaults:
    ...
    def test_portfolio_config_train_bars_default(self):
        cfg = PortfolioConfig()
        assert cfg.train_bars == 8760

    def test_portfolio_config_recal_bars_default(self):
        cfg = PortfolioConfig()
        assert cfg.recal_bars == 2160
...
class TestComputePositionSize:
    """Test 3: Position sizing with ADV cap, max_trade_pct, zero edge."""
    ...
    def test_max_trade_pct_limits_size(self):
        """When max_trade_pct is set, position size should be capped."""
        uncapped = compute_position_size(
            strategy_equity=200_000.0,
            rolling_adv=50_000_000.0,
            volatility=0.02,
            edge=0.10,
            size_multiplier=1.0,
            cap_multiplier=1.0,
        ...
        capped = compute_position_size(
            ...
            size_multiplier=1.0,
            cap_multiplier=1.0,
            max_trade_pct=0.01,  # 1% cap
            adv_cap_pct=0.50,
        )
        assert capped <= 200_000.0 * 0.01 + 0.01
...
# ===================================================================
# 4. Walk-forward masking
# ===================================================================

class TestWalkForwardMasking:
    """Test 4: _apply_walk_forward_mask pattern correctness."""

    def test_first_train_bars_masked(self):
        mask = np.ones(200, dtype=bool)
        result = _apply_walk_forward_mask(mask, train_bars=100, recal_bars=50, purge_bars=10)
        assert not result[:100].any()
        assert result[100 + 10:150].any()  # should be True after purge

    def test_purge_windows_at_recal_points(self):
        mask = np.ones(500, dtype=bool)
        result = _apply_walk_forward_mask(mask, train_bars=100, recal_bars=100, purge_bars=20)
        # Recal points: 100, 200, 300, 400
        # Purge windows: [100,120), [200,220), [300,320), [400,420)
        assert not result[100:120].any()
        assert not result[200:220].any()
        assert not result[300:320].any()
        assert not result[400:420].any()
        # Between purge windows should be True
        assert result[120:200].all()
        assert result[220:300].all()
        assert result[320:400].all()

    def test_no_modification_after_data_end(self):
        mask = np.ones(150, dtype=bool)
        result = _apply_walk_forward_mask(mask, train_bars=100, recal_bars=100, purge_bars=20)
        # Purge window [100,120) masked; [120,150) stays True
        assert not result[100:120].any()
        assert result[120:150].all()

    def test_all_masked_when_train_exceeds_data(self):
        mask = np.ones(50, dtype=bool)
        result = _apply_walk_forward_mask(mask, train_bars=100, recal_bars=50, purge_bars=10)
        assert not result.any()

    def test_original_mask_not_mutated(self):
        original = np.ones(200, dtype=bool)
        original_copy = original.copy()
        _apply_walk_forward_mask(original, train_bars=100, recal_bars=50, purge_bars=10)
...
class TestBuildUnifiedIndex:
    ...
    def test_identical_timestamps(self):
        ts = _make_timestamps(100, "2024-01-01")
    ...
    def test_non_overlapping_timestamps(self):
        ...
        # A has valid bars for first 50 only, rest -1
        assert (bar_maps["A"][:50] >= 0).all()
        assert (bar_maps["A"][50:] == -1).all()
        # B has -1 for first 50, valid for last 50
    ...
    def test_partial_overlap(self):
        ...
        all_signals = {"s30": {"A": sig_a, "B": sig_b}}
        unified, bar_maps = build_unified_index(all_signals)
        # A starts 2024-01-01 for 100h, B starts 2024-01-03 for 100h
        # Overlap is from hour 48 of A through end of A (52 hours)
        # Total unique: 100 + (100-52) = 148
        assert len(unified) == 148
        # A's first 100 bars should map to global bars 0-99
    ...
    def test_same_token_multiple_strategies(self):
        ...
        unified, bar_maps = build_unified_index(all_signals)
        assert len(unified) == 100  # uses longer series
...
class TestSingleTokenSimulation:
    ...
    def test_exit_by_max_hold(self):
        n = 300
        close = np.full(n, 100.0, dtype=np.float64)
        # Keep high/low very tight so stop never triggers
    ...
    def test_exit_by_stop(self):
        ...
        # After no_stop_bars (6), drop the low to trigger stop
        # stop = entry_price - stop_mult * atr = ~100 - 2*2 = 96 (roughly, with slippage)
        for i in range(20, n):
            close[i] = 90.0
            low[i] = 85.0
            high[i] = 91.0
    ...
    def test_exit_by_target(self):
        ...
        high = np.full(n, 100.0, dtype=np.float64)
        low = np.full(n, 100.0, dtype=np.float64)
        # Price goes up after entry to hit target: entry + target_mult * atr = ~100 + 5*2 = 110
        for i in range(20, n):
            close[i] = 115.0
            high[i] = 116.0
            low[i] = 114.0
    ...
    def test_regime_exit(self):
        n = 200
        close = np.full(n, 100.0, dtype=np.float64)
        # Keep high/low tight to avoid stop triggering
        high = np.full(n, 100.01, dtype=np.float64)
        low = np.full(n, 99.99, dtype=np.float64)
        regime = np.zeros(n, dtype=np.int8)
        # Set regime to 1 (unfavorable) after bar 20, with exit_regimes = {1}
        # Entry at bar 10, so by bar 20 bars_held = 10 > 6 (regime exit threshold)
        for i in range(20, n):
            regime[i] = 1

        state = self._run_single_token(
            close, high_array=high, low_array=low,
            entry_bar=10, target_mult=9999.0, max_hold=9999,
            stop_mult=50.0, atr_val=2.0,
            exit_regimes={1}, regime_array=regime,
        )
        trades = state.position_manager.closed_trades
        regime_trades = [t for t in trades if t.exit_reason == "regime"]
...
class TestPortfolioPositionLimits:
    ...
    def test_portfolio_limit_blocks_entry(self):
        ...
        # Three tokens with entries at bar 10 -- only 2 should be allowed
        sigs = {}
        for tok in ["A", "B", "C"]:
            sigs[tok] = _make_token_signals(
                token=tok, n_bars=n, entry_bar=10,
                max_hold=400,  # hold for a long time
...
class TestConcentrationLimit:
    ...
    def test_concentration_blocks_second_entry_same_token(self):
        ...
        # Same token under two strategies
...
# ===================================================================
# 11. Free capital exhaustion
# ===================================================================

class TestFreeCapitalExhaustion:
    """Test 11: Blocking when no capital available."""

    def test_capital_exhaustion_blocks_entry(self):
        n = 500
        # Use moderate capital with high sizing parameters to exhaust capital.
        # Set high concentration_limit so it doesn't block before capital check.
        config = _default_config(
            capital=10_000.0,
            min_position_usd=10.0,
            concentration_limit=1.0,  # 100% -- effectively disable
        )

        sigs = {}
        for tok in ["A", "B", "C", "D", "E", "F", "G", "H", "I", "J"]:
            sigs[tok] = _make_token_signals(
                token=tok, n_bars=n, entry_bar=10,
                max_hold=400, close_price=100.0,
                edge=0.20, adv=500_000_000.0,
                size_multiplier=10.0,  # aggressive sizing to eat capital
                cap_multiplier=10.0,
            )
        all_signals = {"s30": sigs}
        spec = StrategySpec(strategy_id="s30", weight=1.0, max_positions=40)
        state = simulate_portfolio(all_signals, {"s30": spec}, config)
        # With 10 tokens each trying to take a large slice of $10K,
        # some entries must be rejected due to capital exhaustion
        assert state.rejections.capital >= 1
...
class TestCombinedPositions:
    ...
    def test_combined_entry_creates_two_positions(self):
        ...
        sig = _make_token_signals(
            ...
            capital_split=0.5,
            is_perp_primary=False,  # primary is spot
            perp_close=perp_close,
            perp_high=perp_high,
            perp_low=perp_low,
            perp_atr=perp_atr,
        ...
        # Should have 2 trades (primary + secondary)
        assert len(trades) == 2
    ...
    def test_combined_linked_exit(self):
        ...
        # Trigger max_hold exit for primary at bar 10 + 30 = 40
        perp_close = np.full(n, 100.5, dtype=np.float64)
        ...
        trades = state.position_manager.closed_trades
        # Both legs should have exited
        assert len(trades) == 2
        # Both should exit at the same bar
...
class TestFundingAccrual:
    ...
    def test_perp_funding_accrues(self):
        n = 200
        funding_rate = 0.0001  # 1 bps per bar
        sig = _make_token_signals(
            n_bars=n, entry_bar=10,
        ...
        spec = StrategySpec(strategy_id="s30", weight=1.0, max_positions=15)
        state = simulate_portfolio({"s30": {"BTC": sig}}, {"s30": spec}, config)
        # Total funding should be positive (long pays funding when rate > 0)
...
class TestADVCap:
    ...
    def test_tiny_adv_rejects_entry(self):
        ...
        spec = StrategySpec(strategy_id="s30", weight=1.0, max_positions=15)
        state = simulate_portfolio({"s30": {"BTC": sig}}, {"s30": spec}, config)
        # Either the position is too small (min_size) or ADV-capped
...
class TestDeterminism:
    ...
    def test_different_seed_can_differ(self):
        """Different seeds may produce different entry order (via shuffle)."""
        spec = StrategySpec(strategy_id="s30", weight=1.0, max_positions=2)

        config1 = _default_config(seed=42, max_portfolio_positions=2)
        config2 = _default_config(seed=123, max_portfolio_positions=2)

        sigs1 = self._build_multi_token_signals()
        sigs2 = self._build_multi_token_signals()

        state1 = simulate_portfolio({"s30": sigs1}, {"s30": spec}, config1)
        state2 = simulate_portfolio({"s30": sigs2}, {"s30": spec}, config2)

        trades1 = state1.position_manager.closed_trades
        trades2 = state2.position_manager.closed_trades

        # With only 2 positions allowed and 5 candidates at the same bar,
        # different shuffle order means the selected pair may differ.
        # We verify the test runs without error; exact equality is NOT required.
...
class TestCapitalModel:
    ...
    def test_equity_unchanged_while_position_open(self):
        ...
        sig = _make_token_signals(
            n_bars=n, entry_bar=10,
            close_array=close, high_array=high, low_array=low,
            atr_val=2.0, max_hold=9999, target_mult=9999.0,
        )
        config = _default_config()
        spec = StrategySpec(strategy_id="s30", weight=1.0, max_positions=15)
        state = simulate_portfolio({"s30": {"BTC": sig}}, {"s30": spec}, config)
...
class TestEmptySignals:
    ...
    def test_no_entry_signals(self):
        n = 200
        sig = _make_token_signals(n_bars=n, entry_bar=None)  # no entry
        config = _default_config()
        spec = StrategySpec(strategy_id="s30", weight=1.0, max_positions=15)
        state = simulate_portfolio({"s30": {"BTC": sig}}, {"s30": spec}, config)
        assert len(state.position_manager.closed_trades) == 0
...
class TestSingleBar:
    ...
    def test_single_bar_with_entry(self):
        ...
        spec = StrategySpec(strategy_id="s30", weight=1.0, max_positions=15)
        state = simulate_portfolio({"s30": {"BTC": sig}}, {"s30": spec}, config)
        # Entry at bar 0, force-close at end (also bar 0)
        # Either the position is opened and immediately closed, or sizing rejects it
        # The key thing is it doesn't crash
        assert state.portfolio_equity > 0


# ===================================================================
# 19. Edge case: all entries masked by walk-forward
# ===================================================================

class TestAllEntriesMaskedByWalkForward:
    """Test 19: When train_bars > data length, all entries are masked."""

    def test_all_masked(self):
        n = 200
        entry_mask = np.ones(n, dtype=bool)  # all True before masking
        sig = _make_token_signals(n_bars=n, entry_mask_array=entry_mask)
        config = _default_config(train_bars=9999)  # mask everything
        spec = StrategySpec(strategy_id="s30", weight=1.0, max_positions=15)

        # We need to apply the walk-forward mask to the entry_mask before sim.
        # The production code does this in precompute_strategy_signals.
        # For the simulator, we must pass already-masked signals.
        # So let's apply it manually:
        sig.entry_mask = _apply_walk_forward_mask(
            sig.entry_mask, config.train_bars, config.recal_bars, config.purge_bars
        )
...
class TestNaNATR:
    ...
    def test_nan_atr_at_entry(self):
        ...
        spec = StrategySpec(strategy_id="s30", weight=1.0, max_positions=15)
        state = simulate_portfolio({"s30": {"BTC": sig}}, {"s30": spec}, config)
        # Should handle NaN ATR without crashing (falls back to 2% of close)
        assert state.portfolio_equity > 0

    def test_nan_atr_during_position(self):
        n = 200
        atr = np.full(n, 2.0, dtype=np.float64)
        atr[20:30] = np.nan  # NaN ATR during position
        close = np.full(n, 100.0, dtype=np.float64)
...
class TestMinPositionSizeRejection:
    ...
    def test_min_size_rejects_tiny_position(self):
        ...
        sig = _make_token_signals(
            n_bars=n, entry_bar=10,
            edge=0.0001,  # tiny edge -> tiny position
            adv=1_000.0,  # small ADV -> small ADV cap
            close_price=100.0,
        )
        config = _default_config(min_position_usd=10_000.0)  # high minimum
        spec = StrategySpec(strategy_id="s30", weight=1.0, max_positions=15)
        state = simulate_portfolio({"s30": {"BTC": sig}}, {"s30": spec}, config)
...
class TestMultipleStrategiesSameUniverse:
    ...
    def test_two_strategies_independent_tokens(self):
        ...
        state = simulate_portfolio(all_signals, specs, config)
        trades = state.position_manager.closed_trades
        # Both strategies should produce trades
...
class TestNoReentryWhileOpen:
    ...
    def test_no_reentry(self):
        n = 300
        # Entry signals at bar 10 and bar 20 -- second should be blocked
        ...
        state = simulate_portfolio({"s30": {"BTC": sig}}, {"s30": spec}, config)
        trades = state.position_manager.closed_trades
        # Only 1 trade for BTC:s30 since the second entry is blocked
        btc_trades = [t for t in trades if t.token == "BTC" and t.strategy_id == "s30"]
        assert len(btc_trades) == 1


class TestFreeCapitalInvariant:
    """Ensure free capital never goes negative during simulation."""

    def test_free_capital_stays_non_negative(self):
        n = 500
        sigs = {}
        for tok in ["A", "B", "C", "D", "E", "F", "G", "H"]:
            sigs[tok] = _make_token_signals(
                token=tok, n_bars=n, entry_bar=10,
                close_price=100.0, edge=0.10,
                adv=50_000_000.0,
                max_hold=200,
            )
        config = _default_config(capital=50_000.0)
        spec = StrategySpec(strategy_id="s30", weight=1.0, max_positions=40)
        # simulate_portfolio has an invariant check: free_capital >= -0.01
        # If this assertion fails inside the sim, pytest will report it.
        state = simulate_portfolio({"s30": sigs}, {"s30": spec}, config)
        assert state.portfolio_equity > 0
