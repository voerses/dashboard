...
# Constants
# ---------------------------------------------------------------------------
DATA_DIR = os.path.join(os.path.dirname(__file__), "..", "data", "spot", "binance", "daily_ohlcv")
...
def load_all_tokens(data_dir: str, min_days: int = MIN_HISTORY_DAYS) -> pd.DataFrame:
    ...
    frames = []
    files = sorted(f for f in os.listdir(data_dir) if f.endswith("_spot_daily.csv"))
    for fname in files:
        token = fname.replace("_spot_daily.csv", "")
...
def load_btc(data_dir: str) -> pd.DataFrame:
    ...
    path = os.path.join(data_dir, "BTC_spot_daily.csv")
...
def save_results(eval_df: pd.DataFrame, output_dir: str = "outputs"):
