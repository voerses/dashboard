...
"""
Fetch 15-minute spot OHLCV data from Binance Vision (bulk historical archives).
...
Source URL pattern:
    https://data.binance.vision/data/spot/monthly/klines/{SYMBOL}/15m/{SYMBOL}-15m-{YEAR}-{MONTH:02d}.zip
...
Usage:
    python tools/fetch_binance_15m.py                                    # all symbols from 1h_cache
    python tools/fetch_binance_15m.py --symbols BTC ETH SOL             # specific symbols
    python tools/fetch_binance_15m.py --start-year 2023 --end-year 2025 # custom date range
    python tools/fetch_binance_15m.py --force                           # re-download everything
    python tools/fetch_binance_15m.py --workers 8                       # increase parallelism
"""
...
SPOT_1H_CACHE = os.path.join(DATA_DIR, "spot", "1h_cache")
DEFAULT_OUTPUT_DIR = os.path.join(DATA_DIR, "spot", "15m_cache")

# Binance Vision URL pattern
BASE_URL = "https://data.binance.vision/data/spot/monthly/klines"
# {BASE_URL}/{SYMBOL}USDT/15m/{SYMBOL}USDT-15m-{YEAR}-{MONTH:02d}.zip
...
def discover_symbols_from_cache(cache_dir):
    ...
    for f in sorted(os.listdir(cache_dir)):
        if f.endswith("_1h.parquet"):
            ...
            # Skip non-ASCII symbol names (e.g., Chinese characters)
...
def download_monthly_zip(symbol, year, month):
    ...
    Raises on non-404 HTTP errors after retries.
    """
    # Binance Vision uses {SYMBOL}USDT as the pair name
    pair = f"{symbol}USDT"
    url = f"{BASE_URL}/{pair}/15m/{pair}-15m-{year}-{month:02d}.zip"
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if resp.status_code == 404:
                return None  # Not available for this month — normal
            if resp.status_code == 200:
                return resp.content
            # Other errors — retry
            if attempt < MAX_RETRIES - 1:
                wait = BASE_BACKOFF_S * (2 ** attempt)
                print(f"    HTTP {resp.status_code} for {pair} {year}-{month:02d}, "
                      f"retrying in {wait:.0f}s ...", flush=True)
                time.sleep(wait)
            else:
                print(f"    HTTP {resp.status_code} for {pair} {year}-{month:02d} "
                      f"after {MAX_RETRIES} retries, skipping", flush=True)
                return None
        except (requests.ConnectionError, requests.Timeout) as e:
            if attempt < MAX_RETRIES - 1:
                wait = BASE_BACKOFF_S * (2 ** attempt)
                print(f"    Network error for {pair} {year}-{month:02d}: {e}, "
                      f"retrying in {wait:.0f}s ...", flush=True)
                time.sleep(wait)
            else:
                print(f"    Network error for {pair} {year}-{month:02d} "
                      f"after {MAX_RETRIES} retries, skipping", flush=True)
                return None
    return None
...
def parse_zip_csv(zip_bytes, symbol, year, month):
    ...
    try:
        with zipfile.ZipFile(io.BytesIO(zip_bytes)) as zf:
            ...
            if not csv_names:
                print(f"    WARNING: empty ZIP for {symbol} {year}-{month:02d}", flush=True)
                return None
        ...
        print(f"    WARNING: corrupt ZIP for {symbol} {year}-{month:02d}: {e}", flush=True)
        return None

    if df.empty:
        return None

    # Detect timestamp unit: Binance Vision kline archives use microseconds
    # for recent data (16+ digits) but milliseconds for older data (13 digits).
...
def get_resume_start(parquet_path):
    """If parquet exists, return the last timestamp + 15min as resume point.

    Returns (year, month) tuple of the month to start downloading from,
    or None if no resume is possible.
    """
    ...
    try:
        ...
        # Start from the month of the last timestamp (re-download that month
        # to catch any missing data at the end)
...
def fetch_symbol(symbol, start_year, end_year, output_dir, force=False):
    """Fetch all 15m data for a single symbol. Returns (symbol, row_count, status)."""
    parquet_path = os.path.join(output_dir, f"{symbol}_15m.parquet")

    now = datetime.now(timezone.utc)
    end_y, end_m = end_year, now.month if end_year >= now.year else 12
    # Don't go past current month
    if end_year > now.year:
        end_y = now.year
        end_m = now.month
    elif end_year == now.year and end_m > now.month:
        end_m = now.month
    ...
    if not force and os.path.exists(parquet_path):
        ...
        if resume:
            ...
            # If the existing data already covers through end, skip entirely
            ...
            # Parquet exists but can't read — re-download all
            pass

    months = generate_month_list(dl_start_year, dl_start_month, end_y, end_m)
    if not months:
        print(f"  [{symbol}] no months in range, skipping", flush=True)
    ...
    if not all_dfs and not force and os.path.exists(parquet_path):
        # No new data but existing parquet is fine
        existing = pd.read_parquet(parquet_path)
    ...
    if not force and os.path.exists(parquet_path):
        try:
            existing = pd.read_parquet(parquet_path)
            # Existing parquet has timestamp as index; new data has timestamp column
    ...
    # Set timestamp as index (matching 1h parquet schema: datetime64 index, no name)
...
def print_summary(results, elapsed_s):
    ...
    print("\n" + "=" * 70, flush=True)
    print("15M SPOT FETCH SUMMARY", flush=True)
    print("=" * 70, flush=True)
    ...
    print(f"  No data:    {len(no_data)} symbols (not on Binance spot)", flush=True)
    print(f"  Errors:     {len(errors)} symbols", flush=True)

    if no_data:
        print(f"\n  Missing: {', '.join(s for s, _ in no_data)}", flush=True)
    if errors:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Fetch Binance spot 15m OHLCV data from data.binance.vision."
    )
    parser.add_argument(
        "--symbols", nargs="+", default=None,
        help="Symbols to fetch (e.g., BTC ETH SOL). "
             "Default: all symbols from data/spot/1h_cache/."
    )
    parser.add_argument(
...
def main():
    ...
    if args.symbols:
        ...
        symbols = discover_symbols_from_cache(SPOT_1H_CACHE)
        if not symbols:
            print("ERROR: No symbols found in 1h_cache and none specified via --symbols.",
                  file=sys.stderr, flush=True)
            print(f"  Looked in: {SPOT_1H_CACHE}", file=sys.stderr, flush=True)
            sys.exit(1)
        print(f"Discovered {len(symbols)} symbols from 1h_cache", flush=True)

    now = datetime.now(timezone.utc)
    print("=" * 70, flush=True)
    print("Fetch 15M Spot OHLCV — Binance Vision", flush=True)
    print("=" * 70, flush=True)
    print(f"Symbols:    {len(symbols)}", flush=True)
    print(f"Date range: {args.start_year}-01 to {args.end_year}-{now.month:02d}", flush=True)
