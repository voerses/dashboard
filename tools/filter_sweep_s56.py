"""Parameter sweep for anti-crash filters on S56 (24mo, 200K capital).
...
# ── 5. Best combos (top ADV + top DD + top Pump) ──
print(f"\n{'='*60}\n[{STRATEGY_ID}] COMBINATION SWEEP\n{'='*60}", flush=True)

# Find best single filters by DD improvement relative to return cost
...
def score(r):
    """DD improvement per % return sacrificed. Higher = better efficiency."""
    dd_improve = baseline_dd - r['max_dd_hourly_pct']  # positive = better
    ret_cost = max(0.01, baseline_return - r['total_return_pct'])  # positive = worse
    return dd_improve / ret_cost if ret_cost > 0 else dd_improve * 100

# Sort by score, take top 3 of each
...
# Triple combo: best ADV + best DD + best pump
...
# ── Summary table ──
print(f"\n{'='*60}")
print(f"[{STRATEGY_ID}] TOP 10 BY HOURLY MTM DD IMPROVEMENT (with return cost)")
print(f"{'='*60}")
print(f"{'Label':<45} {'Sharpe':>7} {'MaxDD_h':>8} {'DD_imp':>7} {'Return':>8} {'Ret_cost':>9} {'Trades':>7}")
print(f"{'-'*45} {'-'*7} {'-'*8} {'-'*7} {'-'*8} {'-'*9} {'-'*7}")

sorted_results = sorted(results[1:], key=lambda r: r['max_dd_hourly_pct'], reverse=True)  # least negative first = best
