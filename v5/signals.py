"""V5 Portfolio Backtest — Signal precomputation.
...
from v5.engine import Engine, MarketType, _load_strategy_fn, _load_strategy_required_plugins, _load_strategy_required_indicator_groups
...
def precompute_strategy_signals(
    ...
    def _build_ctx(eng, token, df, **kwargs):
        ...
        contamination (e.g. strategy-level context mutation).
        """
        if _use_ctx_cache:
    ...
    for token in tokens:
        try:
            ...
            # Trading should start at anchor - months (walk-forward removed in M1).
            ...
            post_wf_count = int(entry_mask.sum())  # alias for post-liquidity count (walk-forward removed)
