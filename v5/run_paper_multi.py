"""V5 Paper Trading — Multi-portfolio runner.
...
import threading
import time
from v5.paper_config import PaperConfig, validate_paper_config
from v5.config import StrategySpec, SizingDefaults
from v5.paper_engine import PaperPortfolioEngine
from v5.signals import discover_tokens
from v5.paper_utils import restore_state, acquire_pid_lock, compute_sleep_until_next_hour
from v5.dashboard_state import write_dashboard_state
from v5.data_maintenance import ensure_data_fresh
...
def load_multi_config(path: str) -> list[PaperConfig]:
    ...
    for i, pf in enumerate(portfolios):
        ...
        for j, s in enumerate(merged["strategies"]):
            ...
            try:
                from v5.engine import _load_strategy_fn, _STRATEGY_MODULE_CACHE, _STRATEGY_MODULE_LOCK
        ...
        config = PaperConfig(
            ...
            cache_max_rows=merged.get("cache_max_rows", 5000),
        )
...
def main(argv: list[str] | None = None) -> None:
    ...
    try:
        while not shutdown.is_set():
            ...
            try:
                from v5.engine import clear_module_caches
                clear_module_caches()
                import gc
                gc.collect()
            except Exception:
            ...
            # try:
            #     from v5.data_maintenance import discover_perp_1m_tokens
