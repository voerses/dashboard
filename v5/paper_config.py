"""V5 Paper Trading — Configuration and validation.
...
def load_paper_config(path: str) -> PaperConfig:
    ...
    for i, spec in enumerate(strategy_list):
        try:
            from v5.portfolio_backtest import _load_strategy_module_attrs
    ...
    config = PaperConfig(
        ...
        cache_max_rows=data.get("cache_max_rows", 5000),
    )
...
def validate_paper_config(config: PaperConfig) -> None:
    ...
    """
    # --- Bar resolution validation ---
    valid_bar_resolutions = (0, 1, 5, 15, 30)
    if config.bar_resolution not in valid_bar_resolutions:
        raise ValueError(
            f"Invalid bar_resolution {config.bar_resolution}. "
            f"Must be one of: {', '.join(str(r) for r in valid_bar_resolutions)}"
        )
