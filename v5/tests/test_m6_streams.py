"""M6 — DataStream / DataKind / Venue / TransportMode / GapPolicy / Subscription
(T-D14, T-D17 Venue typing, T-D18 Subscription validation non-C3).

C3 price-type capability check lives in test_m6_engine.py.

All tests MUST FAIL today — v5.data.streams does not exist.
"""
...
def _bar_stream():
    ...
    return DataStream(instrument=i, data_kind=DataKind.BAR, bar_spec=BarSpec.from_minutes(60))


def _trade_stream():
    from v5.data.streams import DataKind, DataStream, InstrumentId, Venue
    i = InstrumentId(symbol="BTCUSDT", venue=Venue.BINANCE, asset_class="perp")
    return DataStream(instrument=i, data_kind=DataKind.TRADE, bar_spec=None)


def _funding_stream():
    from v5.data.streams import DataKind, DataStream, InstrumentId, Venue
    i = InstrumentId(symbol="BTCUSDT", venue=Venue.BINANCE, asset_class="perp")
    return DataStream(instrument=i, data_kind=DataKind.FUNDING_RATE, bar_spec=None)
...
class TestEnumMembership:
    """T-D14 / T-D15 / T-D5* — enum membership spot-checks."""

    @pytest.mark.parametrize("name", ["BAR", "TRADE", "FUNDING_RATE", "MARK_PRICE", "INSTRUMENT_INFO"])
    def test_data_kind(self, name):
        from v5.data.streams import DataKind
        assert hasattr(DataKind, name)
...
class TestDataStreamConstructionValidation:
    ...
    def test_bar_without_bar_spec_raises(self):
        ...
        with pytest.raises(ValueError):
            DataStream(instrument=i, data_kind=DataKind.BAR, bar_spec=None)
    ...
    def test_non_bar_with_bar_spec_raises(self):
        ...
        i = InstrumentId(symbol="BTCUSDT", venue=Venue.BINANCE, asset_class="perp")
        with pytest.raises(ValueError):
            DataStream(instrument=i, data_kind=DataKind.FUNDING_RATE,
    ...
    def test_invalid_price_type_rejected(self, bad):
        ...
        with pytest.raises((ValueError, TypeError)):
            DataStream(instrument=i, data_kind=DataKind.BAR,
...
class TestSubscriptionValidation:
    ...
    def test_gap_policy_nan_fill_on_trade_raises(self):
        from v5.data.streams import GapPolicy, Subscription
        with pytest.raises(ValueError):
            Subscription(stream=_trade_stream(), handler=_noop, gap_policy=GapPolicy.NAN_FILL)
