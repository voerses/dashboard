#!/usr/bin/env python3
"""V5 Portfolio Backtest — CLI entry point and orchestrator.
...
from v5.report import compute_portfolio_metrics, print_report, save_results, print_diagnostic_report
...
def _load_strategy_module_attrs(strategy_id: str) -> dict:
    ...
    Returns dict with keys: sizing_overrides, portfolio_config (if present).
    """
    ...
    for fname in os.listdir(strategies_dir):
        if (fname == strategy_id + ".py" or fname.startswith(strategy_id + "_")) and fname.endswith(".py"):
            ...
            if hasattr(mod, 'MAX_POSITIONS_PER_SYMBOL'):
                result['max_positions_per_symbol'] = mod.MAX_POSITIONS_PER_SYMBOL
...
def run_backtest(
    ...
    for sid in strategy_ids:
        ...
        spec = StrategySpec(
            ...
            max_positions_per_symbol=mod_attrs.get('max_positions_per_symbol', 1),
...
def main():
    ...
    if merged_portfolio:
        ...
        print(f"  Effective: max_portfolio_positions={max_portfolio_pos}")
