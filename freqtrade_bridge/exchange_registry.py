...
class ExchangeRegistry:
    """Wrapper class providing a clean interface to the exchange registry functions."""
    ...
    @staticmethod
    def get_supported_tokens(exchange, tokens):
        return get_supported_tokens(exchange, tokens)

    @staticmethod
    def get_pair_whitelist(exchange, tokens):
        return get_pair_whitelist(exchange, tokens)

    @staticmethod
    def get_exchange_availability(tokens):
        return get_exchange_availability(tokens)

    @staticmethod
    def generate_freqtrade_config(exchange, tokens, dry_run=True, capital=200000):
        return generate_freqtrade_config(exchange, tokens, dry_run, capital)
